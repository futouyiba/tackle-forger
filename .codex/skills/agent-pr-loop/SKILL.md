...
Pause and request the missing human decision for an explicit task-scoped user hold on merging; unresolved product or scope semantics; destructive data, security, authorization, secret, billing, legal, or compliance choices; merge-triggered external side effects; unavailable required validation; ambiguous dependency order; a required second GitHub identity; exhausted retries; or an untrustworthy exact-head result. A user instruction at task start or during the task to not merge, wait for a human merge, or ask again before merging remains active until that user later explicitly authorizes the merge. Do not clear it from silence, elapsed time, successful CI or review, retries, or a fresh `READY` result. Do not label ordinary code quality, a completed Agent review, or a generic desire for caution as a human gate.

## Publish and verify a merge

Repository policy decides whether the Agent merges or hands off a PR. In this repository, a trusted live `READY` result for the exact current head/base activates qualified auto-merge standing authorization when no recorded human gate applies. The coordinator must then merge one qualifying PR without requesting a separate per-turn user instruction. A current task-scoped user merge hold is a recorded human gate and takes precedence over `READY` until the user explicitly clears it. If a hold arrives after auto-merge was enabled or the PR entered a merge queue and the PR is still unmerged, immediately disable auto-merge or remove only that PR from the queue and read back its state. If safe cancellation is unavailable, report the human gate and verify whether it merged. Cancellation does not clear the hold; after later explicit authorization, refresh the exact head/base and rerun the live checker before re-entering any merge transport. Use GitHub PR merge, repository auto-merge, or its merge queue as the applicable transport. Immediately before merging, refresh GitHub and re-check the exact head/base, current-run gates, discussions, dependencies, side effects, and the current task for an active user merge hold.
