---
name: agent-pr-loop
description: Orchestrate one GitHub pull request through all comments and review findings, minimal implementation, independent Agent review, current-head CI, and merge eligibility; when the current request explicitly includes a merge, use safe merge and readback. Use when the user explicitly names agent-pr-loop or $agent-pr-loop, or says “搞定 PR”, “搞定这个 PR”, “搞定当前 PR”, “把这个 PR 完成/收尾/处理掉”, “把当前 PR 跑完”, “审完修完这个 PR”, “处理 PR 的评论/review/CI”, “复审后合并”, or “合并收尾”, especially when Codex should infer the active PR instead of requiring its number.
---
...
Declare the PR merge-eligible only when one exact head/base pair has all of:
...
## If merging, publish and verify

When the current request explicitly includes a merge, use GitHub PR merge, repository auto-merge, or its merge queue as the applicable merge transport. Immediately before merging, refresh GitHub and re-check the exact head/base, current-run gates, discussions, dependencies, and side effects. Respect the repository merge method and merge one qualifying PR only.
...
Never deploy, publish, release, delete a branch, or expand scope merely because a PR is merge-eligible.
