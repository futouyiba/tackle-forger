...
Stay inside the configured goal and standing authorization. High-risk pull requests require the repository's strict review and high-risk live gate, but a qualifying merge already covered by `qualified_auto_merge` does not need separate per-turn authorization. An explicit task-scoped user merge hold still takes precedence until the user later authorizes merge. Never expand scope, delete records, change acceptance criteria, handle secrets or billing, perform destructive data changes, deploy, publish, or release without explicit authorization.
