...
Validation evidence is automatically captured from already-executed result JSON as a concise `tackle-validation-summary/v1`: command, artifact/input identity, exit/result, timestamp, and duration. The capture helper never executes a command or shell. It must cover every TaskBrief required command exactly once; PASS cannot contain a failed result. Normal handoffs show only this summary; failure details remain expandable evidence rather than copied logs. Its machine-checked reuse identity binds artifact/version, relevant-input hash, dependency-lock hash (or explicit `none`), command-contract hash, and environment identity; all values must match before reuse. `pr_final` never substitutes a reused local result for exact-head CI.
...
node .codex/skills/tackle-agent-workflow/scripts/workflow-contract.mjs --capture-validation --input <executed-results.json>
