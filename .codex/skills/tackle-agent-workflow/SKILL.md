...
Before implementation or review, follow `docs/spec-v3/README.md`: always read the index, section 0, and sections 19–20; classify the task through one or more routes; then read every routed/relevant section and directly referenced dependency. Read all modules only when scope is unknown, impact is broadly cross-domain, or the canonical specification structure itself changes. Record the base revision and inspect pre-existing changes. Create one compact TaskBrief and pass its raw form to every participating agent:
