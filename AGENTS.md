...
本仓库的managed mode为`autonomous`，作用域是当前仓库和当前明确目标；由单一有权限的coordinator/托管主管负责，活跃任务轮次或已配置唤醒作为heartbeat，当前不声称存在额外后台Automation。重试上限为3个fix-review-CI循环，自动独立review按`reviewTier`和高风险门禁执行，merge policy为`qualified_auto_merge`。合并资格、授权及暂停条件只由`.github/merge-gates.md`定义；部署和发布策略固定为`never`。
...
任何相关远端状态变化后都必须刷新并重跑。不得从被审分支复制checker，也不得通过参数、评论或自身代码自行放行。合并资格、授权、治理例外、用户暂停和合并后的回读全部按`.github/merge-gates.md`执行，不在本文件维护第二份规则。现行仓库不配置GitHub Ruleset、分支保护、required check或额外status context；不得新增重复workflow替代该门禁。合并不授权部署、发布、删除、范围扩张或其他外部副作用。
