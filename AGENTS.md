...
- 验证由工作流的封闭命令目录按 verdict-phase TaskBrief 自行执行并产出精简`command / input identity / exit-result / timestamp / duration`摘要；调用方不得提供成功、哈希、耗时或环境字段。摘要用于交接与复用判断，不能作为可编辑 Verdict JSON 的 PASS 依据；常规交接不得人工复制完整日志，失败详情作为可展开证据保留。复用前必须重新比对工件、相关输入、依赖锁、命令契约和环境身份；PR最终门禁仍以精确head CI为准，不得复用替代。
