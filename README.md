...
产品已确定：内网用户应可直接打开工具，在浏览器中使用本地 Excel 和临时工作区；这些内容不上传、
不读取团队工作区，刷新或关闭页面即失。该匿名本地会话仍待对应运行时实现落地。飞书 OAuth 不再是
入口门槛，但仍是共享工作区、飞书规则源、
保存、发布与正式导出等共享/外部动作的身份边界。纯本地动作只消费随应用版本发布的本地动作契约；
匿名请求任何共享或服务端动作会得到 401 或明确禁用的 `ActionAvailability`，已登录但动作能力不足返回
403。登录失败显示稳定错误编号、重试与管理员入口；
会话过期时浏览器暂存未保存草稿，重登后仅在团队 revision 未变化时自动恢复，否则提供冲突草稿下载。

只有启用共享工作区或飞书规则源时，生产环境才需配置 `.env.example` 中的 OAuth、租户、
会话密钥和支持入口；纯本地会话部署不以这些配置为启动前提。OAuth token、应用密钥不得进入
前端、日志、导出包或 AI 输入。启用 OAuth 时，`FEISHU_SESSION_DATA_DIR` 必须指向受备份保护、
仅服务账号可读写的 R730 持久磁盘。
