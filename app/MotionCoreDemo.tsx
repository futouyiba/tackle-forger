...
import { motionFrozenEvidenceNotice, motionKeyboardCommand, motionLiveAnnouncement, motionStepState, resolveReducedMotion, visibleMotionEvidence, type MotionPreference } from "@/lib/motion-accessibility";
...
export function MotionCoreDemo({ clock = browserMotionClock }: { clock?: MotionClock }) {
  ...
  const visible = visibleMotionEvidence(state.status, model.steps, state.stepIndex);
  const frozenEvidenceNotice = motionFrozenEvidenceNotice(state.status, model.businessRevision, state.revision, model.outputHash);
  ...
  return <main className="motion-core-demo"><section className="motion-core-card" data-reduced-motion={reducedMotion || undefined} aria-labelledby="motion-core-title" onKeyDown={(event) => {
    ...
    {frozenEvidenceNotice ? <p className="motion-frozen-evidence" role="note">{frozenEvidenceNotice} 最终值：<strong>{String(model.finalValue)} kgf</strong>。</p> : null}
