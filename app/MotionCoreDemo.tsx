...
export function MotionCoreDemo({ clock = browserMotionClock }: { clock?: MotionClock }) {
  ...
  return <main className="motion-core-demo"><section className="motion-core-card" aria-labelledby="motion-core-title">
    ...
    <div className="motion-controls" aria-label="演出控制"><button onClick={() => controller.dispatch({ type: "play" })}>播放</button><button onClick={() => controller.dispatch({ type: "pause" })} disabled={state.status !== "playing"}>暂停</button><button onClick={() => controller.dispatch({ type: "resume" })} disabled={state.status !== "paused"}>继续</button><button onClick={() => controller.dispatch({ type: "skip" })}>直接看结果</button><button onClick={() => controller.dispatch({ type: "replay" })}>重播</button><button onClick={() => controller.dispatch({ type: "cancel", reason: "user" })}>取消</button><button onClick={() => controller.dispatch({ type: "revisionChanged", revision: `${model.businessRevision}-fixture-update` })}>模拟 revision 变化</button><label><input type="checkbox" checked={state.reducedMotion} onChange={(event) => controller.dispatch({ type: "reducedMotionChanged", reducedMotion: event.target.checked })}/> 减少动态</label></div>
