...
import { buildV23LocalCopyPayload, executeV23UiAction, previewV23WeightBand, v23CanApplyReadback, v23LatestGeneration, v23WritePreflight } from "@/lib/v23-ui-actions";
...
function PartCard({ part, bandBlocks, orderedBands, state, workspaceRevision, pending, availability, onBand, onSave, write }: {
  part: SeriesPartRevision; bandBlocks: V23BandBlock[]; orderedBands: string[]; state: WorkspaceState;
  workspaceRevision: number; pending: boolean; availability: ActionAvailabilityMap;
  ...
  return <article className={`v23-part-card v23-${part.partType}`}><header><strong>{part.partType.toUpperCase()}</strong><small>immutable {part.partId} · r{part.revision}</small></header>
    ...
    {technologyCatalog.unresolved ? <p className="v23-fail-closed">{technologyCatalog.reason}；已禁用 Technology 挂载。</p> : <div className="v23-technology-actions">{technologyCatalog.technologies.map((technology) => {
      ...
      const attached = part.technologyRefs.some((entry) => entry.id === ref.id && entry.revision === ref.revision && entry.contentHash === ref.contentHash);
      const action = attached ? "remove_part_technology" : "attach_part_technology";
      return <button key={technology.technologyId} type="button" disabled={pending || !availability[action]?.enabled} title={availability[action]?.disabledReasonText} onClick={() => void write(action, technologyPayload(ref), attached ? "Part Technology 已移除并完成 SKU 重算回读。" : "Part Technology 已挂载并完成 SKU 重算回读。")}>{attached ? "移除" : "挂载"} {technology.name}</button>;
    })}{part.technologyRefs.filter((ref) => !technologyCatalog.technologies.some((entry) => entry.technologyId === ref.id && entry.revision === ref.revision && entry.contentHash === ref.contentHash)).map((ref) => <button key={`${ref.id}:${ref.revision}`} type="button" disabled={pending || !availability.remove_part_technology?.enabled} title={availability.remove_part_technology?.disabledReasonText} onClick={() => void write("remove_part_technology", technologyPayload(ref), "旧版 Part Technology 引用已精确移除并完成 SKU 重算回读。")}>移除 {ref.id}@{ref.revision}</button>)}</div>}
...
function SkuCard({ sku, part, state, workspaceRevision, availability, pending, write }: { sku: SkuDrawerRevision; part: SeriesPartRevision; state: WorkspaceState; workspaceRevision: number; availability: ActionAvailabilityMap; pending: boolean; write: (action: Parameters<typeof executeV23UiAction>[0], payload: Record<string, unknown>, message: string) => Promise<void> }) {
  ...
  return <article className="v23-sku-card"><header><strong>{sku.skuId}</strong><small>revision {sku.revision} · {sku.status}</small></header>
    ...
    {technologyCatalog.unresolved ? <p className="v23-fail-closed">{technologyCatalog.reason}；已禁用 SKU Technology 挂载。</p> : <div className="v23-technology-actions">{technologyCatalog.technologies.map((technology) => {
      const ref = { id: technology.technologyId, revision: technology.revision, contentHash: technology.contentHash };
      const attached = sku.technologyRefs.some((entry) => entry.id === ref.id && entry.revision === ref.revision && entry.contentHash === ref.contentHash);
      const inheritedFromPart = part.technologyRefs.some((entry) => entry.id === ref.id);
      const action = attached ? "remove_sku_technology" : "attach_sku_technology";
      return <button key={technology.technologyId} type="button" disabled={pending || inheritedFromPart || !availability[action]?.enabled} title={inheritedFromPart ? "该 Technology 已由 Part 继承，无需在 SKU 重复挂载。" : availability[action]?.disabledReasonText} onClick={() => void write(action, payload({ technologyRef: ref }), attached ? "SKU Technology 已移除并回读。" : "SKU Technology 已挂载并回读。")}>{inheritedFromPart ? "Part 已继承" : attached ? "移除" : "挂载"} {technology.name}</button>;
