...
| [`deployment/r730-production.md`](./deployment/r730-production.md) | 生产运行手册 | Dell R730部署、持久化、备份、验收与回滚；包含工作区revision策略配置、dry-run、归档、恢复验证、首次裁剪与回滚门槛，不定义领域规则 |
| [`audits/aud-009-workspace-revision-retention-adr.md`](./audits/aud-009-workspace-revision-retention-adr.md) | `ACCEPTED`正式决策 | 记录工作区revision分层保留政策、方案比较、容量证据和实施前置条件；规范性要求已进入v3第14.3节，文档接受不表示裁剪已经实现或获准启用 |
