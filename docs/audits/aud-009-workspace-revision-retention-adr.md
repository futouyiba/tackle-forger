...
| 实施分期 | 已确认 | 一期只跑通主流程，不实现归档或裁剪；归档进入二期评估 |
| 归档操作者 | 已确认 | 由当前工具用户主动执行，典型角色为数值策划或系统策划 |
| 优先归档位置 | 二期优先验证 | 通过浏览器保存窗口一键保存到用户工作 PC 自选位置；若简单方案不可行则继续延期 |
| 包格式、压缩/加密、恢复入口与容量限制 | 二期实施参数，尚未确定 | 不阻塞一期；未验证前不得据此裁剪 |
| 恢复目标、告警阈值与维护窗口 | 裁剪前置条件，尚未确定 | 只在未来启用裁剪前完成验证与批准 |
...
以下精确容量数据是 2026-07-23 ADR 起草时记录的**不可复现一次性观察值**：当时没有保存测量 commit、完整命令或脱敏样本数据库标识/hash，因此不能作为回归基线、验收证据或未来容量结论。记录值为：种子状态序列化后单份 JSON 594,961 字节（约 0.57 MiB）；本地 SQLite 的 7 条 revision 平均 482,566 字节、最大 505,516 字节，revision JSON 合计约 3.38 MB。下表只沿用约 0.57 MiB 进行量级演示，SQLite 页、索引和 WAL 还会增加额外开销；后续容量决策必须重新记录完整 commit SHA、命令和脱敏样本数据库hash。
...
| B. 固定条数 | 只保留最新 N 条，例如 100 条 | 空间上界直观；与 Blob 当前行为一致；实现简单 | 时间覆盖随写入频率剧烈变化；高频操作时可能只剩数小时/数天；会删除仍可能需要的调查证据 | 每次成功保存独立提交后另启裁剪事务；首次启用前必须备份并展示待删除范围 |
...
一期目标是先跑通工具主流程。归档和裁剪不进入一期完成标准：

- SQLite/D1 继续保存全部完整 workspace revision；一期不提供归档按钮，不执行裁剪，也不启用自动删除。
- 缺少归档位置、包格式、加密或恢复参数，不得阻断登录、规则、Series/SKU/Model、Snapshot、导出及其他非删除功能。
- 一期可以继续提供只读容量诊断和灾备，但诊断与备份不等于归档成功或裁剪授权。

二期由当前已登录工具用户主动归档，典型操作者是数值策划或系统策划。优先验证以下最简方案：

1. 用户点击“归档”，浏览器立即打开保存窗口；
2. 用户在工作 PC 上选择文件名和保存位置；
3. 工具把完整 revision 和 manifest/hash 流式写入单一归档包；
4. 工具完成写入校验后才显示归档成功并记录操作者、范围、目标和结果。

项目正式入口已经限定 Chromium + HTTPS，因此二期优先使用 File System Access API 的 `showSaveFilePicker()` 和 `FileSystemWritableFileStream`；保存窗口必须由用户手势触发。API 不可用时可以评估普通下载降级，但必须保留同等内容校验。归档包不得包含飞书令牌、登录会话、应用密钥或其他凭据。技术可行性依据见 [Chrome File System Access API 指南](https://developer.chrome.com/docs/capabilities/web-apis/file-system-access)；正式实现仍需用目标浏览器和实际包体做验证。

用户取消、权限拒绝、写入中断、包体超限或校验失败时，不产生成功记录，也不允许裁剪。若本地保存与恢复无法用可接受复杂度完成，归档和裁剪继续延期，不为赶进度引入复杂后台归档系统。

## 5. 裁剪门槛与 fail-closed 行为

以下门槛只适用于二期或更晚的裁剪，不是一期部署门槛。任何自动或人工裁剪都必须同时满足以下条件；任一条件缺失、非法、过期或不可验证时，不得删除任何 revision：

1. `retentionDays=90`、`minimumRevisions=100` 和策略版本通过严格配置校验；实现不得用页面默认值或回退值继续删除。
2. 归档由当前已登录工具用户显式触发，操作者、用户选择的保存目标和导出结果已经形成审计记录。
...
3. SQLite 正常保存继续使用独立的 `BEGIN IMMEDIATE`：只验证 `baseRevision`、更新当前态、插入新 revision，随后 `COMMIT`。只有该提交成功后，才允许通过独立显式命令或任务启动 retention run；即使保存动作请求后续 retention，也不得在保存提交前执行归档预检、证据写入或删除。revision 冲突路径不得启动 retention。
4. SQLite retention run 使用第二个独立事务：重新读取并冻结输入，校验全部归档/备份/恢复/授权证据，原子写入 retention run 与 tombstone 后才删除获准候选。任一预检、归档、证据写入、删除、回读或提交失败只回滚 retention 事务，保留全部候选并记录可观测告警；不得回滚已经成功提交的工作区保存。D1 必须提供等价的事务分离与原子性，或可证明不会产生半完成删除的边界。
5. 每个被裁剪 revision 在删除前写入 tombstone，至少包含 revision、author、message、created_at、状态 JSON 的 SHA-256、pruned_at、策略版本、retention run ID、归档 manifest ID 和已验证备份/恢复标识。
6. retention run 至少记录幂等键、策略版本、冻结 cutoff、输入/保留/候选数量及集合哈希、异常集合、归档证据、备份恢复证据、操作者、开始/结束时间和结果。
7. 相同策略与幂等键重复运行不得重复删除、重复归档或生成重复 tombstone；部分失败重试必须先回读证据。
8. 不在每次保存中执行 `VACUUM`。WAL checkpoint、增量 vacuum 或停服维护由独立运维流程按已批准阈值触发。
...
- 当前备份把数据库、导入文件和飞书会话放在同一目录。二期本地归档包必须与会话备份生命周期隔离且排除凭据；是否压缩/加密、如何共享和访问由二期实现 Issue 评估，不阻塞一期。
- 备份保留期、在线 revision 保留期和归档保留期是三个独立策略。任何一项都不得被另一项的默认值替代。

## 8. 迁移与上线顺序

1. 先完成文档治理；文档 PR 合并不等于 AUD-009 实现完成，也不授权删除。
2. 一期只保留全量 revision、只读容量诊断和现有灾备；不增加归档 UI、裁剪 migration、tombstone/retention run 或自动删除，不因此阻塞主流程上线。
3. 二期先验证用户点击按钮后通过浏览器保存窗口把单一归档包写到工作 PC；确定包格式、大小上限、压缩/加密取舍、manifest/hash 和恢复入口。简单方案不可行时继续延期。
4. 只有用户本地归档与恢复验证可用后，才新增顺序 SQLite/D1 storage migration：建立必要索引、tombstone 表和 retention run 表。schema migration 本身不得删除历史。
5. 实现只读 dry-run、隔离恢复与不可变对象校验；先在现有数据上报告异常时间戳和候选范围。
6. 在隔离副本执行首次裁剪和幂等重跑，验证当前状态、保留 revision、ConfigurationSnapshot hash、Patch/Trace、领域审计和精确历史读取。
7. 在明确授权的维护窗口内执行生产首次裁剪；完成 `PRAGMA integrity_check`、证据核对和回滚演练。是否 vacuum 由空间收益和停机预算单独决定。
8. 只有首次裁剪、恢复和回滚独立验收通过后，才能另行评审是否启用自动裁剪；启用前保持关闭。
9. 上线后监控 revision 总数、90 天写入速率、数据库/WAL/备份/归档体积、裁剪数量、最旧在线时间、最近成功归档和最近恢复演练时间。
...
- 冲突与并发：过期 `baseRevision` 不插入也不启动 retention；并发保存仍只有一个成功；成功保存先独立提交，后续 retention 预检、证据或裁剪失败只保留旧 revision 并告警，不回滚新保存；成功 revision 永远可读取或有可验证归档证据。
...
以下二期问题不影响保留政策的 `ACCEPTED` 状态，也不阻塞一期主流程，但会继续阻止首次裁剪和自动删除。答案必须进入后续 Issue、实现 PR、部署变更和运维证据，不得仅口头约定：

- 单一归档包使用什么格式，最大支持多大，是否压缩或加密？
- `showSaveFilePicker()` 不可用时，普通下载降级能否在目标包体下可靠工作？
- 如何在不读取用户任意目录的前提下完成归档包校验和恢复验证？
- 本地归档由用户自行保管时，是否还需要团队共享位置、访问审批、加密或删除授权？
