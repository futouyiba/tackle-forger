...
- `CandidateSearchRecipe`冻结并引用精确的`constraintSetId + revision + contentHash`和`componentRegistryId + revision + contentHash`；运行时禁止把任一引用解析为后来发布的“最新revision”；
...
CandidateSearchRecipe revision
  → PartConstraintSet(constraintSetId, revision, contentHash)
    → rod constraints
    → reel constraints
    → line constraints
  → ComponentRegistry(componentRegistryId, revision, contentHash)
```
...
- `registryRef`，包含组件注册表的workspace、稳定ID、精确revision和content hash；
- `sourceRef`，包含来源类型、稳定来源ID、来源revision和content hash；
...
```text
提交不可变CandidateSearchRecipe引用
→ 服务端从Recipe revision解析并校验唯一PartConstraintSet/组件注册表revision与hash
...
- 每个具体组件的组件revision、注册表revision、来源revision和hash状态；不能解析的旧引用显示`LEGACY_UNVERSIONED_COMPONENT_REF`，不得伪装成当前组件。
