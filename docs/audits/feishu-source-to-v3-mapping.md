...
4. **09.3 部位占比 / 09.4 全损时间-零整比 当前为 staging_output**。两张表不参与导入（表头未填）；spec §20.1 把「部位占比」「全损时间」「零整比」列为定价公式必填输入。PR2b-3（4e77a3e）后，现行策略从 `33IGHy` 读取维修价/零整比，并为部位占比与全损时间生成值=`1` 的显式身份默认项（均 `status: SOURCE`），构造完整输入，草稿为 `TRIAL_READY` 可进行试算；在显式确认流程把全部输入转为 `CONFIRMED`（当前尚未实现）前，不能正式发布 `PricingPolicyVersion`。未来启用 `34KaIv/35bCfX` 时再通过独立切流替换默认项。
