...
7. 当前`ConfigIdPolicyVersion`引用的`ConfigTargetCatalogVersion`条目和获批`ConfigTargetScanManifest`；Manifest必须覆盖authoritative ref、不可变commit、`config.toml`及受管workbook hashes。正式动作还必须取得`ConfigTargetGovernanceLease`，冻结Manifest set hash、expected old OID、`leaseId`与单调`fencingToken`。
