...
共享工作区、飞书规则源和正式动作的一期验收必须使用真实目标环境、真实公司飞书用户和权威《钓具设计工作簿》。本地测试、mock 和无业务写入预检都只能作为准备证据，不能替代真实环境结论；Vercel、Cloudflare 与 OpenAI Sites 已退役，不属于验收路径。v3 已确定的匿名本地会话尚未由当前运行时代码实现；其独立实现验收必须证明未登录可用、刷新即失、无 SQLite/浏览器持久化/服务器 Action 写入，并逐一证明所有共享路由继续拒绝匿名请求。
...
始终保留 `npm run acceptance:phase-one --` 中的 `--`：它将后续参数交给验收脚本，
而不是交给 Node 的启动参数解析器。验收脚本会安全地把缺失、相对路径、符号链接或权限
不合规的环境文件报告为去敏的 `BLOCKED` 证据，而不是在启动前读取该文件。
