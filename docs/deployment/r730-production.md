...
实现必须提供下列逻辑配置并在启动与每次 retention run 前严格校验。实际配置键由后续实现 PR 和部署示例统一定义；本手册不预先编造环境变量名。
...
| 归档目标与责任人 | 启用前必须明确；当前未在文档中指定 |
| 加密、密钥、访问审批与删除授权 | 启用前必须明确并完成验证 |
...
1. 在裁剪前把每个候选完整 revision 写入已批准的归档目标；归档必须与飞书会话备份隔离，并使用已批准的加密、访问控制和不可变能力。
2. 生成并回读校验归档 manifest，逐项核对 revision、范围、数量、内容 SHA-256、策略版本和归档对象标识。上传成功但未回读验证不算完成归档。
3. 执行当前数据库整库备份并校验 manifest。备份保留期不能替代归档保留期。
4. 在隔离路径恢复备份和至少一个归档 revision，运行 SQLite 完整性检查并核对当前状态、revision、Snapshot hash、Patch/Trace、领域审计与发布记录。不得覆盖生产数据库做恢复演练。
5. 保存恢复耗时、结果、操作者、证据位置和失败处理；结果必须满足已批准的恢复目标。恢复失败时自动裁剪继续关闭。
...
- 在自动裁剪保持关闭的条件下检查 retention 配置和最近 dry-run/归档/备份恢复证据；任一证据缺失时部署可以继续提供不裁剪的工作区服务，但不得启用删除任务。
