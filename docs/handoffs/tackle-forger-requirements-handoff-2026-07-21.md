...
工具部署在公司内网Dell R730，使用飞书登录。按已发布`separation-of-duties/open009-v1`，一期、二期和当前规划三期均使用全员统一的已启用Capability，不建设细粒度RBAC、职责分离或飞书审批；未来变化必须另建Issue并发布新策略版本。关键共享写操作使用带单调fencing token的工作区租约，旧token不得提交本地或远端结果。支持Chromium内核浏览器。
