...
问题统一使用 `ValidationIssue`，动作统一使用受权限约束的 `ActionLink`。每项必须覆盖正常、边界、冲突、恢复、权限和版本冻结验收。
...
- 环境目录下 `xlsx` 表示1001渠道；
- `xlsx/channels/<channel>` 表示同环境其他渠道；
...
写入必须预览差异、校验关系、确认目标、备份并原子替换。关联使用稳定ID和配置键，不按名称、行号或整行覆盖。
...
- AI刷新时机、模型版本和审核权限；
- 钩、浮漂、饵料开放时间；
- Series审核与Model发布是否职责分离；
- 飞书规则提案是否多级会签；
- 定价剩余参数；
- 任何可能改变已发布Snapshot冻结语义的方案。
