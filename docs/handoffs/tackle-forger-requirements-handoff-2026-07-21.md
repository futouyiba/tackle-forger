...
工具部署在公司内网Dell R730，使用飞书登录。按已发布`separation-of-duties/open009-v1`，一期、二期和当前规划三期均使用全员统一的已启用Capability，不建设细粒度RBAC、职责分离或飞书审批；未来变化必须另建Issue并发布新策略版本。关键共享写操作使用带单调fencing token的工作区租约：旧token不得提交服务端状态、服务端可达副作用或成功证据；浏览器本地文件不能由服务端outbox代写，租约失效后按文件冲突与Manifest恢复处理。支持Chromium内核浏览器。
