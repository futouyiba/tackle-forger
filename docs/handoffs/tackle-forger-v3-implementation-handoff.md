...
BaseWeightTemplate
  ...
  → ItemTypeProfile
  → FunctionProfile
  → PerformanceProfile（如有）
  → Attribute/Affix aggregation
  → Layered Patch
  → Validation
```
...
每个 Patch 要保存原因、作者、时间、基于的投影/规则版本和审核状态。上游更新时生成 rebase 预览：旧基础、新基础、现有 Patch、预计结果、冲突；不得自动吞掉冲突。
...
- weightFit：重量适配。
- functionFit：功能定位适配。
- performanceFit：性能定位适配。
- seriesCoherence：系列一致性。
...
验收：选择和购买引用 Model ID 与 Snapshot ID；同一 SKU 可打开并列出多个 Model；系列不变量在每次生成、修改和发布前都能校验。
...
- 其他渠道只使用用户明确选择的具体目录；不扫描目录、不解析或修正`config_system.toml`、不接管渠道治理。
- 多个“环境×渠道”可同时选择，默认让合格目标继续；确认页允许“任一失败则全部不写”。
- File System Access API不可用时提供下载变更包降级，不声称完成本机写入。

### 19.3 配置生成与校验

实现顺序：

1. 定义`ConfigEnvironmentProfile`、`LocalExportTargetBinding`、`ConfigExportMapping`和`ConfigIdBundle`。
2. 实现“准备发布与导出”批次：复用未变化Snapshot、批量冻结合格revision、跳过并报告阻断项，不要求逐个Model确认。
3. 从SnapshotBatch构建`ExportPackage/Manifest`。
4. 在暂存区upsert tackle/item/store，保留未知sheet、列、样式、公式和四行表头。
5. 解析该环境根`config.toml`的`tables.*`与`enums`，复用现有编译器的name-key→数字ID语义。
6. 强制生成部位tackle、item、GoodsBasic和StoreBuy；在所有渠道的StoreBuy schema、配置编译器类型/解析、迁移、导出器和校验器中新增BOOL列`enabled`。新行默认false，更新普通属性时保留各目标现值。
7. 先校验本次变更及其引用闭包；全库关系扫描作为用户主动触发的独立功能。
8. 记录基线hash、生成备份和恢复Manifest，逐文件写入并回读；浏览器跨文件不宣称原子提交。
9. 每目标返回独立结果、hash、备份与审计；不执行任何Git命令。

upsert必须使用稳定`ID + configNameKey`：同名不同ID、同ID不同名或分裂命中均阻止；不得按行号关联、整行覆盖、删除旧行或自动排序。定价公式骨架和部分查表值已位于主飞书工作簿`08_价格计算/u87sRh`；实现`PricingPolicyDraft`导入、试算和Trace可立即开始，但评分插值、零整比、舍入及品质到PricingBasket映射发布前，正式Store导出不得用手填或硬编码价格。
