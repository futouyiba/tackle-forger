...
规则源/飞书
  ...
  → SKU 抽屉/目标拉力
  → 最近结构标杆匹配
  → 功能强度、材料、Patch与词条/Technology
  → 可购买 Model
  → 只读 PerformanceSummary
...
`functionIntensity`、Material、Quality、Affix、Technology和商品层Patch都不得进入最近标杆搜索。Affix/Technology必须位于Series/SKU/Model Patch之后，FinalReviewPatch必须位于词条结算之后；`PerformanceSummary`最后派生且不得反向参与任何计算。不得把“Layered Patch”作为一个可任意移动的模糊阶段。
