...
upsert必须使用稳定`ID + configNameKey`：同名不同ID、同ID不同名或分裂命中均阻止；不得按行号关联、整行覆盖、删除旧行或自动排序。正式ID只能由已发布`ConfigIdPolicyVersion`和reservation ledger分配；OPEN-008关闭前只能做策略接口、示例预览和冲突检查，不得用最大值+1或示例ID正式提交。
