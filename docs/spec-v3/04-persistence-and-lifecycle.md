...
`__TF_PRESERVED`只承载清单允许且不含敏感/raw evidence的冻结根；清单逐根声明records或whole-root singleton载体、record key、revision与hash提取规则。`opaque_canonical_payload_json`只能由服务器导出并exact-compare，永不生成写或移除操作。`__TF_SERVER_REFS`仅为server-owned根保存非敏感root hash和不可重放opaque ref；`__TF_FORBIDDEN`只保存固定`FORBIDDEN_PAYLOAD_OMITTED`标记，严禁从敏感/可猜内容生成hash或ref。`__TF_DIAGNOSTICS`使用版本化closed字段；message/severity为显示信息，整张diagnostic表排除于项目语义hash，诊断变化不得改变项目等价性。

规范化固定为UTF-8、Unicode NFC、LF、RFC 8785 JCS、有限数值、`-0→0`且blank与null不同；记录按根清单顺序再按主键Unicode code point排序。`workbook_schema_sha256`精确覆盖清单中的workbook schema、canonicalization、递归record schema权威、逐根record schemas、冻结根载体目录与93-root分类；record hash覆盖root、schema ID、record key、revision与canonical payload；machine content hash仅覆盖可导入、冻结、server ref与forbidden omission机器表，并排除自身hash单元格、diagnostics及两张派生可读表。语义等价只由规范化machine content SHA-256相等决定。
