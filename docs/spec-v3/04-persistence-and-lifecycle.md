...
即使根属于`importable_current`，其record schema列出的identity、revision、hash、source provenance、发布时间与其他`exactFields`也不能由Excel修改；它们只用于匹配、并发检查和证据比较。OPEN-002历史Performance定义保持只读证据，`technologies.compatiblePerformanceProfileIds`不属于新normalized compatibility输入：无论新建或匹配既有Technology，Excel payload都不得携带该字段，服务端只保留/重派生既有历史关联，字段缺失不构成移除意图。对`v23TechnologyDefinitions`，新记录可声明`itemPartId`，但匹配到已有记录后`itemPartId`必须与服务端值exact-equal。对`purchasableModels`，新记录以及尚未预留`configIdBundleRef`的既有记录仍可按领域动作声明或调整`skuId/stableModelKey/configIdBundleRef`；一旦既有记录已有`configIdBundleRef`，这三项必须共同exact-equal，不能拆换、复用到另一SKU或以Excel改名。只有清单允许且不在无条件或条件式exact策略中的字段才可经已有领域动作进入计划。根内混合生命周期时，任何已发布/已批准/已废弃/已冻结记录均自动提升为exact-equal，缺失不产生removal intent；没有安全领域动作的字段或记录返回`REMOVAL_NOT_SUPPORTED`。不得将根分类解释为直接替换`WorkspaceState[root]`。
