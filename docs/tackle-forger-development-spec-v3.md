...
任何会改变候选资格或候选Snapshot指针的动作都必须使用“同批次原子构建”。包括新增Model、替换`configurationSnapshotId`、最终拉力跨W段，以及`ACTIVE ↔ DEPRECATED/ARCHIVED`导致的候选新增或移除。纯Lifecycle命令即使不创建SnapshotBuild，也必须建立候选差量批次并执行同一分组、取锁和事务契约，不得存在只改Lifecycle、延后重建顶点的旁路。`DEPRECATED ↔ ARCHIVED`两侧均不合格，本身不产生候选变化。空系统、新W段或当前完整身份尚无合法顶点时，该流程同时是唯一合法启动路径；不使用隐藏种子顶点、其他W段数值或人工常量。

SnapshotBatch可以跨Series、SKU和W段，但顶点构建必须先按上述五个字段的完整身份划分`vertexGroupKey`子组。Series不进入顶点身份；不同Series只有在五个身份字段完全相同时才共享子组。不同W段或任一版本字段不同时必须分组，不得共用顶点集合、`vertexSetHash`或预览。

原子构建顺序固定为：

1. 发布命令为本批次每个合法`SnapshotBuild`预分配稳定`snapshotId`；幂等重试必须复用原ID。
2. 分别计算每个Model变更前后的候选资格、冻结最终拉力和`vertexGroupKey`，生成`ADD/REPLACE/REMOVE`候选差量。跨W段或跨定义身份的替换必须展开为旧组`REMOVE`和新组`ADD`，不得只更新其中一组。
3. 收集全部受影响`vertexGroupKey`，按`weightBandId`、`weightBandPolicyVersion`、`fiveAxisDefinitionId`、`fiveAxisDefinitionVersion`、`fiveAxisRuleVersion`的无符号UTF-8字节逐分量字典序取得组锁；不得把分量拼成无长度的字符串后比较。所有会同时修改多组的命令都必须使用这一锁顺序，防止死锁。
4. 每个子组独立读取既有当前正式Snapshot指针集合，再按`modelId`应用该组的候选差量：新Model新增指针，既有Model的新Snapshot替换旧指针，失去`ACTIVE`资格的Model移除指针。目标候选集合中同一Model只能有一个`snapshotId`，不得让被替换或失效的历史Snapshot继续入池。
5. 每个子组独立确定有序候选集合、各轴顶点和`vertexSetHash`，并只用该组顶点计算该组SnapshotBuild的五维预览。有SnapshotBuild的子组在每个必需顶点的轴上都必须至少有一个合法`direct`候选，否则返回`FIVE_AXIS_VERTEX_BOOTSTRAP_INCOMPLETE`。
6. 用户确认后，预检已标记为`ready`的整个SnapshotBatch采用一次数据库事务：所有受影响子组的顶点结果、全部新ConfigurationSnapshot、Model的`configurationSnapshotId`和Lifecycle条件更新、幂等记录及审计证据全部提交或全部回滚。任一子组校验、hash或并发条件失败都整批回滚，不得留下跨组半批成功、无顶点Snapshot、无Snapshot顶点或部分更新指针。
...
若纯Lifecycle移除使某子组的目标候选集合无法为一个或多个必需轴提供合法顶点，该事务必须把子组当前状态原子记为`UNAVAILABLE_NO_ELIGIBLE_CANDIDATE`，冻结空缺轴、目标候选集合hash和原因，并清除“当前可用顶点”指针；不得回退到被移除候选的旧顶点。这不改写任何历史Snapshot或其冻结顶点，但会阻止该子组的新Snapshot发布；后续只能通过同批次加入合法候选重新启动。如同一批次已包含该子组的SnapshotBuild，缺顶点仍按`FIVE_AXIS_VERTEX_BOOTSTRAP_INCOMPLETE`整批回滚，不得提交不可用状态后再发布Snapshot。

同一完整身份子组的并发构建必须串行化，并校验`expectedVertexSetHash`与候选Snapshot指针集合；冲突方回滚并基于新集合重算，不得丢失同时发布的更强装备或Lifecycle变更。新五维定义可以对既有Snapshot冻结输入按完整身份分组重建顶点；若新定义需要的输入在旧Snapshot中未冻结，不得从当前Model补读，而应通过新Model revision的同批次Snapshot启动。

`vertexSetHash`至少覆盖上述完整身份，按`snapshotId + componentEntityId + axisId`稳定排序的全部候选来源身份与冻结直接输入，各轴顶点值和顶点选择规则版本。每次发布顶点集合必须冻结完整候选源列表和hash；重量段策略、五维定义、五维规则、新增更强装备或候选池变化均只能发布新顶点版本并生成UpgradeCandidate，不得复用旧顶点身份或改变历史Snapshot。
...
- 幂等重试复用预分配`snapshotId`并产生同一候选集合和hash；
- 同一W段并发发布发生候选指针或`expectedVertexSetHash`冲突时，失败方整批回滚并重算；
- SnapshotBatch同时包含两个W段时，按完整`vertexGroupKey`构建两个独立顶点集合，每个Model只使用自己子组的`vertexSetHash`；
- 跨W段的同一Model替换展开为旧组REMOVE和新组ADD，任一子组失败时已确认SnapshotBatch整批不提交；
- 相同多组输入以不同SnapshotBuild顺序提交时，仍按完整身份稳定取锁且不死锁，各组候选集合与hash不变；
...
- 移除候选后必需轴无合法值时，当前子组原子进入`UNAVAILABLE_NO_ELIGIBLE_CANDIDATE`且旧顶点不得继续作为当前顶点；历史Snapshot仍可重放，新发布必须先同批次加入合法候选；
...
SnapshotBatch跨Series、SKU或W段时，必须调用第21.3节的多`vertexGroupKey`原子流程。批次预览和确认页至少列出每个子组的完整身份、变更前/后候选集合hash、顶点结果或不可用状态、该组SnapshotBuild及被影响的UpgradeCandidate数量。预检中不合格Model可以在用户确认前明示跳过；一旦确认`ready`集合，所有受影响子组、新Snapshot、Lifecycle/指针更新必须整批单事务提交，不得按W段部分成功。
