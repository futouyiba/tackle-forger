...
策略版本为`ai-model-record/open009-v1`。每次AI调用至少生成供应方与不可变模型描述、promptTemplateVersion、调用人、作用域对象及Revision、input/output hash、证据引用、字段白名单与脱敏策略版本、请求时间、耗时、用量、费用、结果状态、建议、未覆盖信息、假设和采纳状态。保存时必须按第23.6节的字段级矩阵分层，不能把所有字段统称为“调用元数据”。

- provider、modelId、不可变modelRevisionDescriptor、promptTemplateVersion、调用人、作用域引用、input/output hash、策略版本、时间、耗时、用量、费用和结果状态属于操作元数据，保留3年。
...
interface AIModelDescriptorV1 {
  provider: "fancy_hub";
  modelId: SafeCode;
  revisionKind: "model_version" | "deployment_revision" | "artifact_digest";
  revisionValue: SafeCode;
  modelListSnapshotHash: Sha256Hex;
}
```

缺少不可变修订标识的模型不得进入可选列表，产生`AI_MODEL_REVISION_UNAVAILABLE`。主模型不可用时只按已配置顺序跳到下一个同时满足修订要求的模型；列表耗尽时本次评估失败。响应必须回显相同描述，缺失或不一致产生`AI_MODEL_REVISION_MISMATCH`，原始响应只按失败调用留存且不得展示为有效建议或转换草稿。每次评估记录并展示实际`AIModelDescriptorV1`。
...
每一层对象都必须执行`additionalProperties=false`。固定硬上限为：UTF-8序列化后总请求不超过131,072字节；`panelValues<=256`、`traces<=1000`、`patches<=256`、`compatibility<=256`、`affinity<=64`、`invariants<=256`、`fiveAxis<=128`、`evidenceRefs<=256`；任一内部`warningCodes/parameterKeys/conditionCodes<=32`。数字必须有限，整数`sequence`必须在`0..1,000,000`。超出时产生`AI_PAYLOAD_SCHEMA_REJECTED`或`AI_PAYLOAD_LIMIT_EXCEEDED`，要求用户缩小范围，不得截断后静默发送。

禁止发送领域对象中的`name/displayName/label/title/message/reason/assumptions/user_note/excerpt/anchor/url/path/author/createdBy/updatedBy`、真实实体ID、真实Revision、飞书用户ID、任意`unknown`值、自由文本公式、完整规则Payload和ActionLink。硬兼容“原因”只能投影为`ruleCode + conditionCodes`，Evidence只能投影为`evidenceType + evidenceAlias + contentHash`；不得发送正文摘录。当前提示由受控、版本化模板和`analysisIntent`组成，不接受用户自由文本插入。未来要加入任一字段必须发布新Schema与白名单策略版本。

#### 23.6.3 别名、脱敏与发送前检查

每次请求新建从`a001`开始的局部别名表；不同请求不得复用同一实体别名。真实ID、Revision和用户身份只保存在本地加密映射中，不进入Envelope。响应引用无法解析的别名时产生`AI_RESPONSE_ALIAS_UNKNOWN`，对应内容不得生成草稿。

序列化顺序固定为：领域对象→显式安全投影→严格Schema校验→大小/数量校验→密钥扫描→生成canonical JSON→计算inputHash→发送。密钥扫描同时检查已加载凭据的精确值及令牌、私钥、Cookie和Authorization格式；命中时产生`AI_SECRET_DETECTED`并fail-closed。飞书令牌、应用密钥、会话Cookie、Fancy Hub凭据及其他认证材料或密钥在任何情况下都不得进入请求、提示、错误正文、日志或审计正文，且不能被单次操作覆盖。
...
以下边界不可由单次用户确认越过：严格出站Schema、密钥禁发、不可变模型描述、登录与Capability、Fancy Hub返回的provider硬token/并发/速率限制、部署租户的硬费用上限，以及版本化批量策略中的`batchMaxItems`、`batchMaxEstimatedTokens`、`batchMaxEstimatedCost`、`batchMaxConcurrentRequests`和`providerHardTimeoutMs`。启用连接器前这些硬值必须从Fancy Hub能力或部署配置取得且大于0；缺失时产生`AI_HARD_LIMIT_POLICY_MISSING`并禁止调用。批量范围超过硬上限时必须拆分为新的显式批次。

初始软体验阈值为60秒耗时提示、临时错误自动重试1次、每用户同时1个评估、工作区同时4个评估、单次参考输入24k token与输出4k token、工作区每日100次参考量，以及软预算80%和100%提示。超过软阈值但仍低于全部硬准入上限时，显示延迟、排队或成本风险，允许用户继续、排队、再次尝试或主动中断；软阈值不改变发布资格。达到任一硬上限时拒绝新请求或终止在`providerHardTimeoutMs`到期的调用，只影响AI，不影响核心流程。系统记录Fancy Hub能够返回的token、延迟、重试和成本信息。
...
- 被动技能不会被AI当作已验证模拟结果；
- 外部AI请求不包含未授权敏感字段；
- Fancy Hub模型缺少不可变修订时不可选择；主模型失败只按管理员配置顺序降级；响应模型描述不匹配时不能形成有效建议或草稿；
