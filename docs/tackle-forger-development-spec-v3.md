...
- 二期归档必须由已登录数值/系统策划显式点击并选择工作 PC 保存位置；
...
- 比较对象数量上限由UX与前端运行性能配置决定，建议默认2至5件，不作为领域硬常量；
