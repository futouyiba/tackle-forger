...
任何会新增候选Model或替换`configurationSnapshotId`的正式发布都使用“同批次原子构建”。空系统、新W段或当前完整身份尚无合法顶点时，该流程同时是唯一合法启动路径；不使用隐藏种子顶点、其他W段数值或人工常量：

1. 发布命令为本批次每个合法`SnapshotBuild`预分配稳定`snapshotId`；幂等重试必须复用原ID。
2. 顶点构建器在同一个完整身份和W段锁下，先读取既有当前正式Snapshot指针集合，再按`modelId`叠加本批次`ready` SnapshotBuild：新Model新增指针，既有Model的新Snapshot原子替换旧指针。目标候选集合中同一Model只能有一个`snapshotId`，不得让被替换的历史Snapshot与新Snapshot同时入池。
3. 构建器先确定有序候选集合和各轴顶点，再以同一顶点集合计算本批次所有五维预览。每个必需顶点的轴都必须至少有一个合法`direct`候选，否则返回`FIVE_AXIS_VERTEX_BOOTSTRAP_INCOMPLETE`。
4. 顶点集合、本批次全部ConfigurationSnapshot、Model的`configurationSnapshotId`条件更新、幂等记录和审计证据必须在同一数据库事务提交。任一校验、hash或并发条件失败都整批回滚，不得留下无顶点Snapshot、无Snapshot顶点或部分更新指针。

同一完整身份和W段的并发构建必须串行化，并校验`expectedVertexSetHash`与候选Snapshot指针集合；冲突方回滚并基于新集合重算，不得丢失同时发布的更强装备。新五维定义可以对既有Snapshot冻结输入重建顶点；若新定义需要的输入在旧Snapshot中未冻结，不得从当前Model补读，而应通过新Model revision的同批次Snapshot启动。
