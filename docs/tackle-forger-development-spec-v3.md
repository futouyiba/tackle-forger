...
| `configurationSnapshots`、`ruleSetVersions`、`performanceSummaryDefinitions`、`pricingPolicyVersions`、`fiveAxisViewDefinitions`、`fiveAxisVertexSets`、`workspacePolicies`、`candidateRuns`、`candidateMaterializations` | 已发布或不可变的版本、候选和冻结历史 | 相应发布、候选生成或物化命令；Snapshot只读/发布边界。PerformanceSummaryDefinition当前只读，专用版本化发布命令尚未提供 |
