...
- PatchSetHash；
- finalPanelValues；
- 新Snapshot只接受完整的版本化`componentSelections`，包括组件revision、注册表revision、来源revision、各级content hash与值快照；历史Snapshot可以保留旧裸payload但不得用于构建新Snapshot；
...
- Recipe绑定：请求只能提交精确Recipe引用；服务端从Recipe revision解析唯一约束集与组件注册表，客户端不能拼接其他revision；兼容输入携带冗余引用时，类型、workspace、ID、revision或hash任一不一致均fail-closed；
- 排序绑定：排序版本和排序键只来自Recipe revision；规范请求不携带排序字段，兼容冗余字段不相等时fail-closed；`recipeInput`出现冻结引用、排序、阈值或检查点覆盖键时拒绝；
- 组件冻结：候选、Model和新Snapshot中的每个组件选择都冻结组件revision、注册表revision、来源revision和内容hash；注册表发布新revision后旧结果仍可逐字节重放；
...
- 旧组件引用：判别联合的`LEGACY_UNVERSIONED_COMPONENT_REF`分支原样保存`componentId/name/values`与rawPayload，且只允许历史读取/诊断；Candidate、物化、新Model revision和新Snapshot的类型与服务端校验都只接受`VERSIONED_COMPONENT_REF`；
...
interface EntityRef {
  ...
  entityType: "collection" | "series" | "sku_drawer" | "model"
    | "candidate_search_recipe" | "part_constraint_set" | "component_registry"
    | "configuration_snapshot" | "model_candidate" | "adjustment_patch"
    | "upgrade_candidate" | "rule_source_change_draft";
  entityId: string;
  revisionId: string;
}
interface ImmutableEntityRef extends EntityRef {
  contentHash: string;
}
type CandidateSearchRecipeRef = ImmutableEntityRef & {
  entityType: "candidate_search_recipe";
};
type PartConstraintSetRef = ImmutableEntityRef & {
  entityType: "part_constraint_set";
};
type ComponentRegistryRevisionRef = ImmutableEntityRef & {
  entityType: "component_registry";
};
interface ComponentSourceRevisionRef {
  sourceType: string; sourceId: string; revisionId: string; contentHash: string;
}
interface VersionedModelComponentSelection {
  referenceKind: "VERSIONED_COMPONENT_REF";
  itemPartId: string;
  componentId: string;
  componentRevisionId: string;
  componentContentHash: string;
  registryRef: ComponentRegistryRevisionRef;
  sourceRef: ComponentSourceRevisionRef;
  nameSnapshot: string;
  valuesSnapshot: Record<string, number | string>;
  selectionContentHash: string;
}
interface LegacyUnversionedModelComponentSelection {
  referenceKind: "LEGACY_UNVERSIONED_COMPONENT_REF";
  itemPartId: string;
  componentId: string;
  name: string;
  values: Record<string, number | string>;
  rawPayload: Record<string, unknown>;
  migrationDiagnosticCodes: string[];
}
type ModelComponentSelection =
  | VersionedModelComponentSelection
  | LegacyUnversionedModelComponentSelection;
...
```ts
type CandidateRecipeInputValue =
  | string | number | boolean | null
  | string[] | number[] | boolean[];
interface CandidateRecipeInputDefinition {
  inputKey: string;
  valueType: "string" | "number" | "boolean" | "string_list" | "number_list" | "boolean_list";
  required: boolean;
}
interface CandidateSearchRecipeRevision {
  ref: CandidateSearchRecipeRef;
  partConstraintSetRef: PartConstraintSetRef;
  componentRegistryRef: ComponentRegistryRevisionRef;
  sortDefinitionVersion: string;
  runtimeInputDefinitions: CandidateRecipeInputDefinition[];
  // 搜索范围、阈值、检查点、排序键/方向及允许的运行时收紧策略也属于ref.contentHash覆盖的不可变内容
}
interface CandidateGenerationRequest {
  requestId: string; seriesRef: EntityRef; skuRefs: EntityRef[];
  recipeRef: CandidateSearchRecipeRef;
  recipeInput: Record<string, CandidateRecipeInputValue>;
...
interface ModelCandidate {
  ...
  componentSelections: VersionedModelComponentSelection[];
...
规范请求不携带独立`partConstraintSetRef`、`componentRegistryRef`或`sortDefinitionVersion`。服务端先校验`recipeRef`的workspace、`entityType=candidate_search_recipe`、稳定ID、精确revision和content hash，再读取不可变Recipe revision中的唯一`partConstraintSetRef`、`componentRegistryRef`、`sortDefinitionVersion`与完整排序键/方向；随后校验引用和排序定义均存在且hash可验证。任一引用不存在、类型错误、hash不符或跨workspace时，在枚举前以BLOCKER fail-closed。兼容适配器如果仍接收旧线协议中的冗余`partConstraintSetRef`或`sortDefinitionVersion`，只能把它们当一致性断言：前者必须与Recipe内引用在workspace、entityType、ID、revision和hash上全部相等，后者必须与Recipe冻结值精确相等；否则分别返回`CANDIDATE_CONSTRAINT_REF_MISMATCH`或`CANDIDATE_SORT_DEFINITION_MISMATCH`，不得覆盖Recipe或继续生成。

`recipeInput`只承载Recipe revision的`runtimeInputDefinitions`已声明的标量或同类标量数组；服务端必须拒绝未声明key、类型/范围不符和值结构不合法。`partConstraintSetRef`、`componentRegistryRef`、`sortDefinitionVersion`、排序键/方向、阈值、检查点及其别名/嵌套形式属于保留配置，禁止出现在`runtimeInputDefinitions`或`recipeInput`；命中时返回`CANDIDATE_RECIPE_INPUT_OVERRIDE_FORBIDDEN`。`enabledVariantKeys/perSkuLimit/minimumAffinity/acceptWarnings`是显式运行时收紧项，只能按Recipe冻结的允许策略缩小范围、降低数量或提高门槛，不能放宽Recipe；Recipe未声明可收紧时请求值必须与冻结值一致。

`inputHash`由服务端对Series/SKU引用、完整Recipe内容、Recipe内约束集/组件注册表引用与内容hash、Recipe冻结的排序定义、RuleSet/Patch revision、验证后的`recipeInput`和允许的运行时收紧项进行规范序列化后重算；客户端提供值仅作并发断言。重算不一致返回`CANDIDATE_INPUT_HASH_MISMATCH`，不得通过信任客户端hash绕过引用或排序校验。

输入据此冻结Series/SKU/Recipe/PartConstraintSet/组件注册表/RuleSet/Patch revision。系统先按rod/reel/line分别枚举组件，再应用template、material、required affix和有效的部位type硬过滤；optional affix pool只按Recipe冻结的扩展/排序定义参与。之后组合具体组件，执行杆轮线闭环硬兼容、Affinity、Series不变量和稳定排序。deny/缺require只进排除统计。权威排序为Recipe冻结的版本化字典序：配方键→warning数→Affinity降序→拉力距离升序→fingerprint；请求、`recipeInput`和AI均不得改写。结果含部位级约束来源、命中/排除原因、未知引用、排除分组、枚举总数、截断、版本、hash、耗时。每项`componentSelections`必须是第24.1节的`VersionedModelComponentSelection`；`itemPartId/componentId/componentRevisionId/componentContentHash/registryRef/sourceRef`必须与冻结注册表记录逐项一致，`selectionContentHash`必须通过重算。CandidateRun是不可变审计产物，候选不是Model。

默认行为是自动物化：对每个`SKU × enabledModelVariantKey`选取排名最高的合法候选，将候选中实际选定的完整版本化`componentSelections`原样写入一个新建或更新的Model草稿revision。物化命令和新Model revision的写入类型只接受`VersionedModelComponentSelection[]`；出现`LEGACY_UNVERSIONED_COMPONENT_REF`时返回`LEGACY_COMPONENT_REF_NOT_MATERIALIZABLE`。物化前再次解析CandidateRun冻结的Recipe/约束集/注册表并验证所有组件revision与hash；注册表已有新revision不使旧引用自动失效，但旧冻结revision无法解析、组件不属于该revision或hash不符时返回`COMPONENT_REVISION_REF_INVALID`并阻止物化。搜索约束本身不得写入Model。用户可通过Recipe允许的范围、重量、启用路线、每SKU数量、最低Affinity、warning接受和检查点收紧批量生成，不得覆盖Recipe排序或放宽门槛。若`skuId + modelVariantKey`唯一命中旧Model，则创建新revision；无命中新建；多重或歧义命中则跳过并报Issue，禁止按name猜测。内容hash未变化时不创建空revision。同输入、版本与算法必须产生相同结果和顺序，正常流程不使用random seed。
...
| 阶段 | 必做 | 明确不做 |
| --- | --- | --- |
| 一期 | 公司内网部署、飞书登录、统一Capability接口、核心规则/Series/SKU/Model/Snapshot、配置表生成与关系校验 | AI运行连接器、细粒度RBAC |
...
一期面向所有支持File System Access API的Chromium内核浏览器，默认通过HTTPS提供内网页面。无内网DNS且明确接受降级时，可通过默认关闭的显式配置，仅对RFC 1918私网IP开放HTTP；此时会话Cookie不设置Secure，且浏览器依赖安全上下文的能力（包括File System Access API）可能不可用。用户通过目录选择器显式授权本地配置仓库；一期不要求安装本地伴随程序，也不让服务器访问设计人员电脑。

```ts
interface ConfigEnvironmentProfile {
  environmentId: string;       // 例如 dev、test
  label: string;
...
- dev、test是环境，不是渠道；每个环境对应一个独立configs仓库根目录，并使用自己的`config.toml`。
...
- `FileSystemDirectoryHandle`保存在该用户、该浏览器、该站点origin的IndexedDB中；Cookie仅用于飞书登录会话。
...
正常路径：选择dev/test中的多个显式目标，在暂存区生成三表，关系全通过后确认并提交，各目标返回新hash与备份。  
