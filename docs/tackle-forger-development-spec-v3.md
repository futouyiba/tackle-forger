...
| OPEN-010 飞书Patch台账远端契约 | 外部规则源阻断 | `BLOCKED_ON_SOURCE_SCHEMA` | 已确认`Patch台账/edyFx9`、`A:AK`机器区、`AM:BA`协作事件区、哈希/并发/幂等/补偿/rebase契约；权限引用`separation-of-duties/open009-v1`；本地PatchLedger和契约测试可运行 | 远端表头、保护边界和连接器联调完成前，真实镜像写入/拉取保持禁用；不得伪造SYNCED | 按第14.4节物化远端schema；完成写入、重复重试、回读、缺行、篡改、hash和并发冲突联调；连接器实现另建Issue/PR |
