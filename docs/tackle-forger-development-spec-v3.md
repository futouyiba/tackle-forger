...
| 分类 | 当前根数 | 工作簿语义 |
| --- | ---: | --- |
| `importable_current` | 24 | 可按领域命令导入的当前可变项目数据；每个根必须命中机器清单中的closed record schema，逐记录校验稳定ID、revision、引用与不变量。 |
...
| `server_owned` | 21 | 工作区身份、schema、Config ID治理、Patch台账、固定品质映射、既有Series与v23 affix命令边界、Part/SKU/Technology head pointer、含raw迁移证据的约束集、飞书来源派生policy draft、含legacy raw payload的定价版本、外部拉取草稿、幂等与审计等服务端权威；工作簿只绑定服务端生成的非敏感hash/reference，不能携带可回写payload。 |
...
即使根属于`importable_current`，匹配既有记录时record schema列出的全部`revisionFields`必须自动加入exact比较，再与identity、hash、source provenance、发布时间、其他`exactFields`及适用的conditional exact fields共同验证；nested path按真实声明类型取值并以canonical typed value比较，number与string不混同，revision缺失或篡改均阻断。新建记录通常只做closed payload校验，不因不存在existing revision而误阻断；但清单可对命令专属实体声明create拒绝策略。`skuDrawers`即属于该例外：工作簿不得新建SKU，新增必须显式执行v23 `create_sku`；匹配既有SKU时只有`targetPullKg`可变，其他受支持字段全部exact-equal，且不得改变`v23SkuDrawerRevisions`冻结证据或`v23SkuDrawerHeads`服务端head。OPEN-002历史Performance定义保持只读证据，`technologies.compatiblePerformanceProfileIds`不属于新normalized compatibility输入：无论新建或匹配既有Technology，Excel payload都不得携带该字段，服务端只保留/重派生既有历史关联，字段缺失不构成移除意图。对`v23TechnologyDefinitions`，新记录可声明`itemPartId`，但匹配到已有记录后`itemPartId`必须与服务端值exact-equal。对`purchasableModels`，新记录以及尚未预留`configIdBundleRef`的既有记录仍可按领域动作声明或调整`skuId/stableModelKey/configIdBundleRef`；一旦既有记录已有`configIdBundleRef`，这三项必须共同exact-equal，不能拆换、复用到另一SKU或以Excel改名。只有清单允许且不在revision、无条件或条件式exact策略中的字段才可经已有领域动作进入计划。根内混合生命周期时，任何已发布/已批准/已废弃/已冻结记录均自动提升为exact-equal，缺失不产生removal intent；没有安全领域动作的字段或记录返回`REMOVAL_NOT_SUPPORTED`。不得将根分类解释为直接替换`WorkspaceState[root]`。
