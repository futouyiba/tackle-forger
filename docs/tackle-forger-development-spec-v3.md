...
Model最终参数、组件或相关Patch变化后必须重算，包括最终拉力变化导致W段切换。发布Snapshot冻结最终拉力、W段及策略版本、五维定义ID/版本、三条部件曲线、三条结构投影参考曲线、完整有序`FiveAxisVertexCandidateSource`列表、`hashInputSchemaVersion`及`vertexSetHash`、输入哈希、飞书源revision和`fiveAxisRuleVersion`。上游顶点变化只能生成UpgradeCandidate。
