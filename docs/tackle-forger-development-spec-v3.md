...
| OPEN-006 AI供应方与数据出网 | 安全/产品决策 | `RESOLVED` | 使用`ai-provider/open006-v1`：Fancy Hub、动态模型列表、版本化字段白名单和可配置软运行阈值 | 本决策只解除产品策略阻断；真实连接器在独立实现Issue完成、测试并启用前继续禁用，不得发送真实数据 | 2026-07-23用户确认本节策略；AI无批准、写回或发布能力，无需另设三方会签 |
...
| OPEN-009 工作流治理策略 | 产品/安全决策 | `RESOLVED` | 使用`open009-2026-07-23-v1`四类策略；所有已登录公司用户拥有全部已启用业务Capability；AI一期禁用，二期连接器仍需独立实现准入 | 不接飞书审批、不在本工具实行职责分离；OPEN-006安全配置只由部署管理员修改；关键写操作使用工作区单写锁，普通操作记录保留1年 | 2026-07-23用户确认；策略正文见第20.2节，迁移与验收见Issue #18 |
...
策略版本为`ai-model-record/open009-v1`。每次AI调用至少保存供应方、模型标识与版本、promptTemplateVersion、调用人、作用域对象及Revision、input/output hash、证据引用、字段白名单与脱敏策略版本、请求时间、耗时、用量、费用、结果状态、建议、未覆盖信息、假设和采纳状态。

- 上述调用元数据保留3年。
- 实际发送的白名单输入、完整提示和原始输出加密保存180天，到期自动删除；原始内容不进入普通操作记录，查看原始内容本身也要形成操作记录。
- 被人工采纳的建议、人工修改差异、assessmentId及其生成的Patch或规则草稿来源随对应产物永久保留。
...
本节使用策略版本`ai-provider/open006-v1`。AI连接器只接入公司内网Fancy Hub，Tackle Forger不直接连接外部模型供应方。模型列表在运行时从Fancy Hub取得，不在代码或规范中写死具体模型。管理员从Fancy Hub当前返回的列表中配置一个主模型和有序降级列表；主模型不可用时可以自动降级，但不得选择列表外或未配置的模型。每次评估必须记录并展示Fancy Hub返回的实际provider、modelId和可获得的version。降级列表耗尽时只停止本次AI评估，不得影响核心流程。
...
- 最终面板数值；
- 脱敏后的Calculation Trace；
- Patch操作链；
- 硬Compatibility结果和失败原因；
- Affinity各轴贡献；
- Series不变量和重量曲线；
- 五维图单件曲线、短板汇总和比较结果；
- RuleSetVersion、Revision、Snapshot和证据的必要引用。

稳定实体ID在请求中转换为本次请求使用的一次性代号，响应返回后由Tackle Forger本地映射。姓名、飞书用户ID等身份字段不是绝对禁区，但当前没有分析必要，因此不进入默认白名单。完整飞书文档、工作簿原始行、附件和未分类自由文本因体积、敏感性和单次AI调用无法渐进读取而不进入默认白名单。本机路径、`config.toml`、配置仓库文件、配置表和导出包同样不进入默认白名单。任何新增字段默认禁止发送；只有管理员发布新版白名单策略后才能进入请求。

飞书令牌、应用密钥、会话Cookie、Fancy Hub凭据及其他认证材料或密钥是绝对禁止项，在任何情况下都不得进入AI请求、提示、错误正文、日志或审计正文，且不能被单次操作覆盖。

保留和删除策略固定为：

- Tackle Forger本地加密保存脱敏输入、提示和原始输出180天；
- provider、model、promptTemplate和策略版本等元数据保留3年；
- 结构化Finding、证据引用、采纳或忽略反馈以及普通操作记录保留1年；
- 已形成ModelPatch、RuleSourceChangeDraft或其他正式产物的来源关系随对应产物长期保留；
- 用户删除未采纳评估后立即停止展示，备份副本最多30天清除；
- 审计事件、权限和可见性复用OPEN-009的统一治理策略，不在OPEN-006建立第二套审计语义。

初始运行参数为60秒耗时提示、网络/限流/临时服务错误自动重试1次、每用户同时1个评估、工作区同时4个评估、单次参考输入24k token与输出4k token、工作区每日100次参考量，以及预算80%和100%提示。它们全部是版本化、可配置的软阈值，不是发布门禁或硬限制。超过阈值时显示延迟、并发或成本风险，允许用户继续等待、再次尝试或主动中断；不得因这些阈值停止核心业务流程。系统记录Fancy Hub能够返回的token、延迟、重试和成本信息。

所有通过公司身份认证的用户都可以运行AI、查看建议并把建议转换为允许的ModelPatch或RuleSourceChangeDraft草稿。只有部署配置认定的管理员可以修改provider、主模型、降级顺序、字段白名单、保留策略和软运行参数。AI永远只能提供解释、建议和草稿，不能批准Patch、确认warning、写回飞书、发布RuleSet、发布ConfigurationSnapshot或修改已发布Snapshot。AI连接器本身不执行发布，因此本策略不要求额外的安全、产品和数据负责人会签；本次用户决策记录和Issue记录构成决策证据。
...
- 被动技能不会被AI当作已验证模拟结果；
- 外部AI请求不包含未授权敏感字段；
- Fancy Hub模型列表变化不会触发未配置模型的静默替换；主模型失败时只按管理员配置顺序降级并记录实际模型；
- 字段序列化器默认拒绝未知字段，实体稳定ID只以请求级代号发送；
- 任意密钥出现在候选请求字段、提示或错误对象时，连接器fail-closed且不发送请求；
- 超过耗时、并发、token、次数或预算软阈值时只提示并允许继续或中断，不改变发布资格；
- 普通用户不能修改provider、白名单、保留或运行策略；
- 保留到期、用户删除和30天备份清理可验证，已采纳产物的来源关系不被清理。

### OPEN-006：AI供应方与数据出网策略

2026-07-23已确认并关闭产品决策，策略版本为`ai-provider/open006-v1`：使用公司内网Fancy Hub；模型从provider动态读取并由管理员配置主模型与降级顺序；请求严格采用版本化字段白名单和请求级实体代号；密钥绝对禁发；上游训练、留存和地域不另设限制；保留、删除、软运行阈值、权限和审计按第23.6节执行。

关闭OPEN-006不等于真实连接器已经实现或获准启用。本Issue只固化安全与产品策略；连接器实现、契约测试、密钥扫描、白名单验证、失败降级和上线准入必须另建Issue和Pull Request。在这些验证完成前，真实AI连接器继续保持禁用，只允许本地假数据和契约测试。
...
| 阶段 | 必做 | 明确不做 |
| --- | --- | --- |
| 一期 | 公司内网部署、飞书登录、统一Capability接口、全员统一权限、核心规则/Series/SKU/Model/Snapshot、配置表生成与关系校验 | AI运行连接器、细粒度RBAC、职责分离、飞书审批 |
