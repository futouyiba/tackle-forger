...
- `traces`：`subjectAlias, parameterKey, sequence, layerCode, sourceAlias, sourceVersionAlias, operationCode, JCS(element)`；
- `patches`：`scopeType, subjectAlias, parameterKey, operation, JCS(element)`；
...
- 对同一业务输入随机重排数据库返回集合、实体遍历顺序、全部顶层数组及内部code数组至少100次，每次都产生完全相同的别名布局、canonical Envelope字节和inputHash；跨请求允许重新出现`a001`，但映射对象、真实引用或存储实例不得复用；
