...
| pricing | 超300M为需二次确认的WARNING；字段不可表示仍是BLOCKER | 查看价格Trace、确认超限、修复目标schema |
...
冻结 Snapshot 提供三个独立动作，不得合并为一个“导出”入口：

- 查看：`view_snapshot`，要求`snapshot.read`；以冻结payload只读展示，不重算。
- 下载原样审计归档：`download_snapshot_audit_archive`，要求`snapshot.audit_archive.download`；缺策略引用但冻结payload与自身hash仍可验证时允许下载，并在Manifest标记`replayStatus=UNVERIFIABLE_POLICY_REFERENCE`。
- 正式导出：`export_snapshot`，要求`snapshot.export`；缺策略引用、不可重放、Trace/hash不一致或其他完整性BLOCKER时禁用，且不能由审计归档入口绕过。

复制新 Revision、生成升级候选仍是独立动作；Snapshot不可原地编辑、重算、rebase、换 hash、删除引用、被 AI 或上游覆盖。失败 SnapshotBuild 可幂等重试，不产生半快照。
