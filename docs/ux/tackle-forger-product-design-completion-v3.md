...
部署在公司内网 Dell R730。飞书登录建立公司会话；令牌/密钥不得进入前端日志、AI 输入或导出。一期使用统一公司Capability策略且AI禁用；二期只有OPEN-006关闭后才启用AI；当前规划三期继续统一权限，不建设细粒度RBAC、职责分离或飞书审批。关键共享写入显示锁持有人和动作；租约失效后旧fencing token的结果必须被拒绝并进入回读恢复。登录失败提供重试、错误编号、管理员入口；会话过期保留表单，重登后重验 Revision。
