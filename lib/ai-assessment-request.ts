...
export const AI_ASSESSMENT_PROMPT_VERSION = "tackle-forger-assessment-v1";
...
export function buildWorkspaceAssessmentEnvelope(input: {
  ...
  if (input.scope.scopeType === "series") {
    ...
    const aliases = createRequestAliasMap([assessmentRef, seriesRef, revisionRef]);
    ...
    return {
      ...
      traces: [], patches: [], compatibility: [], affinity: [], invariants: [], fiveAxis: [], evidenceRefs: [],
    };
  }
  ...
  const aliases = createRequestAliasMap([assessmentRef, modelRef, revisionRef]);
