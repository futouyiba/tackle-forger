...
const GOVERNED_STATE_FIELDS: readonly GovernedStateField[] = [
  ...
  // Phase A has no generic mutation endpoint for these revision/evidence roots.
  { field: "v23SeriesPartRevisions", reason: "domain_command", action: "POST /api/v23/actions（create_series / update_part_configuration）", actionLabel: "使用 v23 Part 领域动作" },
  { field: "v23SeriesPartHeads", reason: "domain_command", action: "POST /api/v23/actions（create_series / update_part_configuration）", actionLabel: "使用 v23 Part 领域动作" },
  ...
  { field: "canonicalRuleSourceDrafts", reason: "audit_or_reserved_identity", action: "POST /api/feishu-workbook（pull；create_ruleset_draft / publish_ruleset）", actionLabel: "显式拉取工作簿后创建并发布 RuleSet", route: "/api/feishu-workbook" },
  { field: "weightTemplatePolicyDrafts", reason: "audit_or_reserved_identity", action: "POST /api/feishu-workbook（pull；create_ruleset_draft / publish_ruleset）", actionLabel: "显式拉取重量模板后创建并发布 RuleSet", route: "/api/feishu-workbook" },
  ...
  { field: "aiRuleSourceChangeDrafts", reason: "audit_or_reserved_identity", action: "POST /api/ai/assessments/:assessmentId/drafts；POST /api/ai/rule-source-change-drafts/confirm", actionLabel: "通过 AI 草稿创建与人工确认写回流程处理" },
  { field: "aiArtifactProvenanceSyncRecords", reason: "audit_or_reserved_identity", action: "POST /api/ai/assessments/:assessmentId/drafts（创建草稿及永久来源同步/重试）", actionLabel: "通过 AI 草稿创建或同一幂等键重试来源同步" },
