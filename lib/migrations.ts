  ...
  for (const value of skus) {
    ...
    if (entry.derivation !== undefined) {
      ...
      if (derivationStatus === "UNRESOLVED") v23ExactKeys(derivation, ["status"], "V23_SKU_DERIVATION");
        ...
        for (const stepValue of v23Array(derivation.trace, "V23_SKU_DERIVATION_TRACE")) { const step = v23Record(stepValue, "V23_SKU_DERIVATION_STEP"); v23ExactKeys(step, ["source", "operationId", "operationIndex", "operation", "direction", "magnitude", "clampMin", "clampMax", "ratioOperations", "flatComponents", "beforeKg", "afterKg", "numericEvidence"], "V23_SKU_DERIVATION_STEP"); if (!Number.isFinite(step.beforeKg) || !Number.isFinite(step.afterKg) || step.beforeKg !== prior || (step.afterKg as number) <= 0) throw new Error("V23_SKU_DERIVATION_TRACE_INVALID"); prior = step.afterKg as number; }
