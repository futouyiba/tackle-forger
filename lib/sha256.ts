// Small synchronous SHA-256 implementation for deterministic hashes shared by
// browser and server code. Input is always UTF-8 text.
const SHA256_K = [
...
export function sha256Text(text: string): string {
  ...
  for (let shift = BigInt(56); shift >= BigInt(0); shift -= BigInt(8)) {
    bytes.push(Number((bitLength >> shift) & BigInt(0xff)));
  }
  const hash = [
