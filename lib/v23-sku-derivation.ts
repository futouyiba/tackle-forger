...
export interface V23PullTraceStep { affixId: string | null; operationId: string; operationIndex: number | null; operation: "set" | "percent_adjust" | "flat_adjust" | "clamp_add"; direction: "increase" | "decrease" | null; magnitude: number; clampMin: number | null; clampMax: number | null; ratioOperations: Array<{ affixId: string; operationId: string; operationIndex: number; direction: "increase" | "decrease"; magnitude: number }> | null; flatComponents: Array<{ affixId: string; operationId: string; operationIndex: number; direction: "increase" | "decrease"; magnitude: number; numericEvidence: V23PullTraceStep["numericEvidence"] }> | null; flatDeltaEvidence: V23PullTraceStep["numericEvidence"] | null; beforeKg: number; afterKg: number; numericEvidence: { beforeBinary64: string; afterBinary64: string; exactNumerator: string; exactDenominator: string; anomaly: "none" | "overflow" | "underflow_to_zero" }; }
...
function anomaly(value: Rational, result: number): "overflow" | "underflow_to_zero" | null { const numerator = value.numerator < z ? -value.numerator : value.numerator; const denominator = value.denominator < z ? -value.denominator : value.denominator; if (!Number.isFinite(result) || denominator === z || numerator * maxFinite.denominator > maxFinite.numerator * denominator) return "overflow"; return value.numerator !== z && result === 0 ? "underflow_to_zero" : null; }
...
export function deriveV23SkuPull(baselinePullKg: number, entries: readonly V23ResolvedAffix[], options: V23DerivationOptions = {}): V23SkuPullDerivation {
  ...
  if (setOperations.length === 1) { const selected = setOperations[0]!; const set = selected.operation as { value: unknown }; if (typeof set.value !== "number" || !Number.isFinite(set.value) || set.value <= 0) return invalid("V23_AFFIX_SET_INVALID", "set", value, typeof set.value === "number" ? set.value : value, exact(typeof set.value === "number" ? set.value : value), selected.entry, selected.operation); const before = value; value = set.value; trace.push({ affixId: selected.entry.ref.id, operationId: selected.operation.operationId, operationIndex: selected.operation.operationIndex, operation: "set", direction: null, magnitude: set.value, clampMin: null, clampMax: null, ratioOperations: null, flatComponents: null, flatDeltaEvidence: null, beforeKg: before, afterKg: value, numericEvidence: evidence(before, value, exact(set.value)) }); }
  ...
  if (ratioOperations.length) {
    ...
    trace.push({ affixId: first.affixId, operationId: first.operationId, operationIndex: first.operationIndex, operation: "percent_adjust", direction: first.direction, magnitude: first.magnitude, clampMin: null, clampMax: null, ratioOperations, flatComponents: null, flatDeltaEvidence: null, beforeKg: ratioBefore, afterKg: value, numericEvidence: evidence(ratioBefore, value, ratioExact) });
  }
  const flat = later.filter(({ operation }) => operation.operation === "flat_adjust");
  let increasePool = 0; let decreasePool = 0;
  const flatComponents: NonNullable<V23PullTraceStep["flatComponents"]> = [];
  for (const { entry, operation } of flat) { const numeric = operation as Extract<typeof operation, { direction: "increase" | "decrease" }>; const before = numeric.direction === "increase" ? increasePool : decreasePool; const nextExact = add(exact(before), exact(numeric.magnitude)); const next = Number(before + numeric.magnitude); const flatAnomaly = anomaly(nextExact, next); if (flatAnomaly) return invalid(flatAnomaly === "overflow" ? "V23_BINARY64_OVERFLOW" : "V23_BINARY64_UNDERFLOW_TO_ZERO", "flat_pool", before, next, nextExact, entry, operation); if (numeric.direction === "increase") increasePool = next; else decreasePool = next; flatComponents.push({ affixId: entry.ref.id, operationId: numeric.operationId, operationIndex: numeric.operationIndex, direction: numeric.direction, magnitude: numeric.magnitude, numericEvidence: evidence(before, next, nextExact) }); }
  if (flat.length) { const beforeKg = value; const deltaExact = add(exact(increasePool), { numerator: -exact(decreasePool).numerator, denominator: exact(decreasePool).denominator }); const delta = Number(increasePool - decreasePool); const deltaEvidence = evidence(increasePool, delta, deltaExact); const deltaAnomaly = anomaly(deltaExact, delta); const last = flat.at(-1)!; if (deltaAnomaly) return invalid(deltaAnomaly === "overflow" ? "V23_BINARY64_OVERFLOW" : "V23_BINARY64_UNDERFLOW_TO_ZERO", "flat_settlement", increasePool, delta, deltaExact, last.entry, last.operation); const nextExact = add(currentExact, exact(delta)); value = Number(value + delta); const flatAnomaly = anomaly(nextExact, value); if (flatAnomaly) return invalid(flatAnomaly === "overflow" ? "V23_BINARY64_OVERFLOW" : "V23_BINARY64_UNDERFLOW_TO_ZERO", "flat_settlement", beforeKg, value, nextExact, last.entry, last.operation); currentExact = exact(value); trace.push({ affixId: null, operationId: "v23:flat-settlement", operationIndex: null, operation: "flat_adjust", direction: null, magnitude: delta, clampMin: null, clampMax: null, ratioOperations: null, flatComponents, flatDeltaEvidence: deltaEvidence, beforeKg, afterKg: value, numericEvidence: evidence(beforeKg, value, nextExact) }); }
