...
import {
  ...
  FANCY_HUB_RESPONSE_BYTE_LIMITS,
  FancyHubConnector,
  FancyHubError,
...
test("单模型首次临时失败后自动重试同一修订且成功", async () => {
  ...
  const transport = new MockTransport(discoveryModels(), [transient, response(primary)]);
  ...
  const connector = new FancyHubConnector(
    ...
    transport,
    (event) => { auditedActor = event.actorStableId; },
  );
  ...
  assert.deepEqual(result.attemptedModelIds, ["model.alpha", "model.alpha"]);
  assert.equal(result.response.model.modelId, "model.alpha");
...
test("同一模型两次临时失败后才降级到下一模型", async () => {
  const discovered = describeFancyHubModels(discoveryModels().models);
  const transient = () => new FancyHubError("AI_PROVIDER_TEMPORARILY_UNAVAILABLE", "temporary", true);
  const fallback = discovered.models.find((entry) => entry.modelId === "model.beta")!;
  const transport = new MockTransport(discoveryModels(), [transient(), transient(), response(fallback)]);
  const rawAttempts: Parameters<NonNullable<Parameters<FancyHubConnector["assess"]>[0]["rawAttemptSink"]>>[0][] = [];
  const result = await new FancyHubConnector(connectorConfig(), transport).assess({
    workspaceId: "w1",
    actorStableId: "ou-test",
    buildEnvelope: envelope,
    rawAttemptSink: (attempt) => { rawAttempts.push(attempt); },
  });
  assert.deepEqual(result.attemptedModelIds, ["model.alpha", "model.alpha", "model.beta"]);
  assert.equal(result.response.model.modelId, "model.beta");
  assert.deepEqual(transport.calls, ["models", "assess", "assess", "assess"]);
  assert.deepEqual(rawAttempts.map((attempt) => attempt.modelDescriptor.modelId), [
    "model.alpha",
    "model.alpha",
    "model.beta",
...
test("非临时 provider 失败不重试也不降级", async () => {
  const invalid = new FancyHubError("AI_FANCY_HUB_RESPONSE_INVALID", "invalid", false);
  const transport = new MockTransport(discoveryModels(), [invalid]);
  await assert.rejects(
    new FancyHubConnector(connectorConfig(), transport)
      .assess({ workspaceId: "w1", actorStableId: "ou-test", buildEnvelope: envelope }),
    (error) => error === invalid,
  );
...
test("模型发现对同一已配置 modelId 的多个修订 fail-closed，不发评估请求", async () => {
  const transport = new MockTransport({
    ...discoveryModels(),
    models: [
      { modelId: "model.alpha", modelVersion: "v1", deploymentRevision: "deploy.1" },
      { modelId: "model.alpha", modelVersion: "v2", deploymentRevision: "deploy.2" },
      { modelId: "model.beta", deploymentRevision: "d2" },
    ],
  }, [response(modelList())]);
  await assert.rejects(
    new FancyHubConnector(connectorConfig(), transport)
      .assess({ workspaceId: "w1", actorStableId: "ou-test", buildEnvelope: envelope }),
    (error) =>
      error instanceof FancyHubError
      && error.code === "AI_FANCY_HUB_RESPONSE_INVALID",
  );
...
test("共享准入按调用人统计生产 in-flight，并把用户软并发提示返回而不阻断", async () => {
  const coordinator = new InMemoryFancyHubAdmissionCoordinator();
  let releaseFirst!: () => void;
  let markFirstEntered!: () => void;
  const firstEntered = new Promise<void>((resolve) => { markFirstEntered = resolve; });
  const firstRelease = new Promise<void>((resolve) => { releaseFirst = resolve; });
  const firstTransport: FancyHubTransport = {
    async listModels() { return discoveryModels(); },
    async assess(input) {
      markFirstEntered();
      await firstRelease;
  ...
  const first = new FancyHubConnector(
    connectorConfig({ fallbackModelIds: [] }),
    firstTransport,
    undefined,
    coordinator,
    ...
    workspaceId: "w1",
    actorStableId: "ou-same-user",
    buildEnvelope: envelope,
  });
  await firstEntered;

  const secondPrimary = describeFancyHubModels(discoveryModels().models).models
    .find((model) => model.modelId === "model.alpha")!;
  const secondTransport = new MockTransport(discoveryModels(), [response(secondPrimary)]);
  const second = await new FancyHubConnector(
    connectorConfig({ fallbackModelIds: [] }),
    secondTransport,
    undefined,
    coordinator,
  ...
  assert.deepEqual(second.softWarnings, ["AI_USER_SOFT_CONCURRENCY"]);
  assert.deepEqual(secondTransport.calls, ["models", "assess"]);
...
test("评估准入等待到绝对期限时 fail-closed，且不调用 assess transport", async () => {
  const originalNow = Date.now;
  let now = 1_000;
  Date.now = () => now;
  let consumptionCount = 0;
  let assessCalls = 0;
  const coordinator: FancyHubAdmissionCoordinator = {
    async readProviderHardLimits() { return undefined; },
    async writeProviderHardLimits() {},
    async acquire() {
      return {
        inFlightForUserBefore: 0,
        inFlightForWorkspaceBefore: 0,
        inFlightTotalBefore: 0,
        async consumeAssessmentRequest() {
          consumptionCount += 1;
          if (consumptionCount === 2) now = 601_000;
        },
        async release() {},
      };
    },
  ...
  const transport: FancyHubTransport = {
    ...
    async assess() {
      assessCalls += 1;
      throw new Error("不应调用评估 transport");
    },
  };
  try {
    await assert.rejects(
    ...
    assert.equal(consumptionCount, 2);
...
test("评估准入等待未过期时使用等待后的精确剩余时间收紧 timeout", async () => {
  const originalNow = Date.now;
  let now = 1_000;
  Date.now = () => now;
  let consumptionCount = 0;
  const assessmentTimeouts: number[] = [];
  const coordinator: FancyHubAdmissionCoordinator = {
    async readProviderHardLimits() { return undefined; },
    async writeProviderHardLimits() {},
    async acquire() {
      return {
        ...
        async consumeAssessmentRequest() {
          consumptionCount += 1;
          if (consumptionCount === 2) now = 591_000;
        },
        async release() {},
      };
    },
  ...
  const transport: FancyHubTransport = {
    async listModels() { return discoveryModels(); },
    async assess(input) {
      assessmentTimeouts.push(input.timeoutMs);
      return response(input.model);
    },
  };
  try {
    await new FancyHubConnector(
      connectorConfig({ fallbackModelIds: [] }),
      transport,
      undefined,
      coordinator,
    ...
    assert.equal(consumptionCount, 2);
    assert.deepEqual(assessmentTimeouts, [10_000]);
  } finally {
    Date.now = originalNow;
  }
});
...
function exactJsonResponseBytes(byteLength: number): string {
  const prefix = "{\"padding\":\"";
  const suffix = "\"}";
  return `${prefix}${"x".repeat(byteLength - Buffer.byteLength(prefix) - Buffer.byteLength(suffix))}${suffix}`;
}

function controlledOversizedResponse(input: { totalBytes: number; status?: number; chunkBytes?: number }) {
  const chunkBytes = input.chunkBytes ?? 4_096;
  let emittedBytes = 0;
  let pulls = 0;
  let cancellations = 0;
  const body = new ReadableStream<Uint8Array>({
    pull(controller) {
      pulls += 1;
      if (emittedBytes >= input.totalBytes) {
        controller.close();
        return;
      }
      const length = Math.min(chunkBytes, input.totalBytes - emittedBytes);
  ...
  return {
    response: new Response(body, { status: input.status ?? 200 }),
    stats: () => ({ emittedBytes, pulls, cancellations }),
  };
}

test("HTTP transport 对成功、错误和模型列表正文执行流式字节硬上限，边界值正常", async () => {
  for (const [path, byteLimit] of [
    ["models", FANCY_HUB_RESPONSE_BYTE_LIMITS.modelList],
    ["assessment", FANCY_HUB_RESPONSE_BYTE_LIMITS.assessment],
  ] as const) {
    const boundaryBody = exactJsonResponseBytes(byteLimit);
    const boundary = new FetchFancyHubTransport(
      "https://fancy-hub.internal/",
      "token",
      (async () => new Response(boundaryBody, { status: 200 })) as typeof fetch,
    );
    if (path === "models") {
      assert.equal((await boundary.listModels({ timeoutMs: 1_000 }) as { padding: string }).padding.length > 0, true);
    } else {
    ...
    const oversized = controlledOversizedResponse({ totalBytes: byteLimit + 64 * 1_024 });
    const limited = new FetchFancyHubTransport(
      "https://fancy-hub.internal/",
      "token",
      (async () => oversized.response) as typeof fetch,
    );
    const invoke = path === "models"
      ? () => limited.listModels({ timeoutMs: 1_000 })
      : () => limited.assess({ canonicalJson: "{}", inputHash: "a".repeat(64), model: modelList(), maxOutputTokens: 1, timeoutMs: 1_000 });
    await assert.rejects(invoke, (error) => {
      if (!(error instanceof FancyHubError) || error.code !== "AI_FANCY_HUB_RESPONSE_LIMIT_EXCEEDED") return false;
      const raw = error.rawResponse as FancyHubTruncatedRawResponseV1;
      assert.equal(raw.schemaVersion, "fancy-hub-truncated-response/v1");
      assert.equal(raw.endpoint, path === "models" ? "model_list" : "assessment");
      assert.equal(raw.byteLimit, byteLimit);
      assert.equal(raw.capturedBytes, byteLimit);
      assert.equal(Buffer.from(raw.prefixBase64, "base64").byteLength, byteLimit);
      assert.equal(raw.truncated, true);
      return true;
    });
    const stats = oversized.stats();
    assert.equal(stats.cancellations, 1);
    assert.equal(stats.emittedBytes < byteLimit + 64 * 1_024, true);
  }

  const boundedError = new FetchFancyHubTransport(
    "https://fancy-hub.internal/",
    "token",
    (async () => new Response("bounded-error", { status: 503 })) as typeof fetch,
  );
  await assert.rejects(
    boundedError.assess({ canonicalJson: "{}", inputHash: "a".repeat(64), model: modelList(), maxOutputTokens: 1, timeoutMs: 1_000 }),
    (error) => error instanceof FancyHubError
      && error.code === "AI_PROVIDER_TEMPORARILY_UNAVAILABLE"
      && error.rawResponse === "bounded-error",
  );

  const oversizedError = controlledOversizedResponse({
    totalBytes: FANCY_HUB_RESPONSE_BYTE_LIMITS.assessment + 1,
    status: 503,
  });
  const limitedError = new FetchFancyHubTransport(
    "https://fancy-hub.internal/",
    "token",
    (async () => oversizedError.response) as typeof fetch,
  );
  await assert.rejects(
    limitedError.assess({
      canonicalJson: "{}",
    ...
    (error) => error instanceof FancyHubError
      && error.code === "AI_FANCY_HUB_RESPONSE_LIMIT_EXCEEDED"
...
test("超限评估响应零有效建议，并形成有界且可稳定加密的失败 raw record", async () => {
  const oversized = controlledOversizedResponse({
    totalBytes: FANCY_HUB_RESPONSE_BYTE_LIMITS.assessment + 64 * 1_024,
  });
  let fetchCalls = 0;
  const transport = new FetchFancyHubTransport(
    "https://fancy-hub.internal/",
    ...
    (async () => {
      fetchCalls += 1;
      return fetchCalls === 1 ? Response.json(discoveryModels()) : oversized.response;
    }) as typeof fetch,
  );
  const rawAttempts: Parameters<NonNullable<Parameters<FancyHubConnector["assess"]>[0]["rawAttemptSink"]>>[0][] = [];
  await assert.rejects(
    new FancyHubConnector(connectorConfig({ fallbackModelIds: [] }), transport).assess({
      workspaceId: "w1",
      actorStableId: "ou-test",
      buildEnvelope: envelope,
      rawAttemptSink: (attempt) => { rawAttempts.push(attempt); },
    }),
    (error) => error instanceof FancyHubError && error.code === "AI_FANCY_HUB_RESPONSE_LIMIT_EXCEEDED",
  );
  assert.equal(rawAttempts.length, 1);
  assert.equal(rawAttempts[0]?.resultCode, "AI_FANCY_HUB_RESPONSE_LIMIT_EXCEEDED");
  const truncated = rawAttempts[0]?.rawResponse as FancyHubTruncatedRawResponseV1;
  assert.equal(truncated.capturedBytes, FANCY_HUB_RESPONSE_BYTE_LIMITS.assessment);
  assert.equal("result" in truncated, false);
  assert.equal(oversized.stats().cancellations, 1);

  const directory = await mkdtemp(path.join(tmpdir(), "tackle-forger-ai-response-limit-"));
  const key = randomBytes(32);
  const previous = {
    dataDir: process.env.AI_RETENTION_DATA_DIR,
    key: process.env.AI_RETENTION_ENCRYPTION_KEY_BASE64,
  ...
  try {
    const store = createAIRuntimeStoreFromEnvironment();
    const record = store.failedAssessmentRecord({
      assessmentId: "assessment-response-limit",
      actorStableId: "ou-test",
      operationMetadataContext: {
        scopeType: "model",
        scopeId: "model-1",
      ...
      requestAliasMapping: [{
        alias: "a001",
        reference: { referenceKindCode: "assessment", stableLocalId: "assessment-response-limit" },
      }],
      rawAttempts,
    });
    await store.saveAssessment(record);
    ...
    assert.equal(persisted?.metadata?.resultCode, "AI_FANCY_HUB_RESPONSE_LIMIT_EXCEEDED");
    assert.equal(persisted?.semanticContent, undefined);
    const retained = JSON.parse(decryptAIRawContent({
      assessmentId: "assessment-response-limit",
      encrypted: persisted!.encryptedRawContent!,
      key,
    })) as { attempts: Array<{ rawModelResponse: FancyHubTruncatedRawResponseV1 }> };
    assert.deepEqual(retained.attempts[0]?.rawModelResponse, truncated);
    assert.equal(Buffer.from(retained.attempts[0]!.rawModelResponse.prefixBase64, "base64").byteLength,
...
test("真实运行时在响应返回前持久化审计事件与加密留存记录", async () => {
  ...
  try {
    ...
    await lease.release();
    const firstUserLease = await coordinatorA.acquire({
      workspaceId: "default",
      actorStableId: "ou-user-1",
      maxConcurrentForWorkspace: 4,
      maxConcurrentTotal: 4,
      leaseExpiresAtMs: Date.now() + 60_000,
    });
    const secondUserLease = await coordinatorB.acquire({
      workspaceId: "default",
      actorStableId: "ou-user-1",
      maxConcurrentForWorkspace: 4,
      maxConcurrentTotal: 4,
    ...
    assert.equal(firstUserLease.inFlightForUserBefore, 0);
    assert.equal(secondUserLease.inFlightForUserBefore, 1);
    await secondUserLease.release();
    await firstUserLease.release();
    ...
    const record = store.successfulAssessmentRecord({
      ...
      requestAliasMapping: [
        ...
        {
          alias: "a008",
          reference: { referenceKindCode: "evidence", stableLocalId: "trace:model-1", stableRevisionId: "1" },
        },
      ],
      rawAttempts: [{
        requestedAt: "2026-07-23T00:00:00.000Z",
    ...
    assert.equal(persisted.metadata?.metadataSchemaVersion, "ai-operation-metadata/v2");
    assert.deepEqual(persisted.metadata?.scope, {
      scopeType: "model",
      scopeId: "model-1",
      inputRevision: "1",
    });
    assert.equal(persisted.metadata?.ruleSetVersion, "rules-v1");
    assert.equal(persisted.metadata?.fiveAxisRuleVersion, "five-axis-v1");
    assert.equal(persisted.metadata?.attempts?.[0]?.attemptKind, "INITIAL");
    assert.equal(persisted.metadata?.attempts?.[0]?.resultCode, "SUCCESS");
    assert.equal(persisted.metadata?.retryCount, 0);
    assert.equal(persisted.metadata?.cancellationStatus, "NOT_REQUESTED");
    assert.equal(persisted.metadata?.resultCode, "SUCCESS");
    assert.deepEqual(persisted.semanticContent?.resolvedEvidenceRefs, [{
      evidenceType: "trace",
      evidenceAlias: "a008",
      refId: "trace:model-1",
      revisionId: "1",
    ...
    assert.deepEqual(failedPersisted.metadata?.scope, {
      scopeType: "model",
      scopeId: "model-1",
      inputRevision: "1",
    });
    assert.equal(failedPersisted.metadata?.ruleSetVersion, "rules-v1");
    assert.equal(failedPersisted.metadata?.fiveAxisRuleVersion, "five-axis-v1");
    assert.equal(failedPersisted.metadata?.attempts?.[0]?.resultCode, "AI_MODEL_REVISION_MISMATCH");
    assert.equal(failedPersisted.metadata?.cancellationStatus, "NOT_REQUESTED");
    ...
    await store.sweepRetention({
      now: new Date("2027-01-20T00:00:04.000Z"),
      backupAdapter: {
        async purgeAssessmentBackups() {},
        async verifyAssessmentBackupsAbsent() { return true; },
      },
    });
    const semanticAfterRawExpiry = await store.readAssessmentForActor({
      assessmentId: "assessment-1",
      actorStableId: "ou-user-1",
    });
    assert.equal(semanticAfterRawExpiry?.encryptedRawContent, undefined);
    assert.deepEqual(semanticAfterRawExpiry?.semanticContent?.resolvedEvidenceRefs, [{
      evidenceType: "trace",
      evidenceAlias: "a008",
      refId: "trace:model-1",
      revisionId: "1",
