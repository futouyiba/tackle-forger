...
test("真实连接器默认关闭、配置或硬限额缺失时 fail-closed 且 transport 计数为 0", async () => {
  ...
  await assert.rejects(
    connector.assess({ workspaceId: "w1", buildEnvelope: envelope }),
...
test("不同 connector 实例共享原子准入协调，第二个并发请求在 transport 前被拒绝", async () => {
  ...
  const first = new FancyHubConnector(constrained, blockingTransport, undefined, coordinator)
    .assess({ workspaceId: "w1", buildEnvelope: envelope });
  await entered;
  await assert.rejects(
  ...
  assert.deepEqual(secondTransport.calls, ["models"]);
  releaseFirst();
  await first;
});
...
test("响应结果递归拒绝未知字段、越界正文与请求外别名，并在成功前生成 outputHash", async () => {
  ...
  for (const [raw, code] of [
    ...
    await assert.rejects(
      new FancyHubConnector(connectorConfig(), transport).assess({ workspaceId: "w1", buildEnvelope: envelope }),
