...
test("GET/DELETE assessment 路由不泄露非所有者记录并保持删除幂等", { concurrency: false }, async () => {
  ...
  const request = (
    method: "GET" | "DELETE",
    openId: string,
    assessmentId = "assessment-route",
  ) => new NextRequest(`http://localhost/api/ai/assessments/${assessmentId}`, {
    method,
    headers: {
      "x-feishu-tenant-key": "tenant",
      "x-feishu-open-id": openId,
  ...
  const context = (assessmentId = "assessment-route") => ({
  ...
  try {
    ...
    const assertOwnerSeesStaleHistory = async (assessmentId: string) => {
      const retainedHistory = await getAssessment(
        request("GET", "route-owner", assessmentId),
        context(assessmentId),
      );
      assert.equal(retainedHistory.status, 200);
      const retainedPayload = await retainedHistory.json() as {
        metadata?: { assessmentId?: string };
        semanticContent?: { findings?: unknown[] };
        freshness: {
          state: string;
      ...
      assert.equal(retainedPayload.metadata?.assessmentId, assessmentId);
      assert.ok(retainedPayload.semanticContent);
      assert.deepEqual(retainedPayload.freshness, {
        state: "stale",
        canCreateDraft: false,
        staleReasonCodes: ["AI_SCOPE_NOT_FOUND"],
      });
    };

    for (const [index, missingScope] of ([
      { assessmentId: "assessment-route-missing-model", scopeType: "model", scopeId: "model-deleted-from-workspace" },
      { assessmentId: "assessment-route-missing-series", scopeType: "series", scopeId: "series-deleted-from-workspace" },
    ] as const).entries()) {
      await store.saveAssessment(indexedRecord({
        ...missingScope,
        actorStableId: "route-owner",
        createdAt: `2026-07-23T0${index + 2}:00:00.000Z`,
      }));
      await assertOwnerSeesStaleHistory(missingScope.assessmentId);
      assert.equal(
        (await getAssessment(
          request("GET", "other-user", missingScope.assessmentId),
          context(missingScope.assessmentId),
        )).status,
        404,
      );
    }
    ...
    const seriesWithSku = workspace.state.seriesDefinitions.find((series) =>
      workspace.state.skuDrawers.some((sku) => sku.seriesId === series.id));
    assert.ok(seriesWithSku);
    const originalSeriesItemPartId = seriesWithSku.itemPartId
      ?? workspace.state.skuDrawers.find((sku) => sku.seriesId === seriesWithSku.id)?.projectionMatch.itemPartId;
    assert.ok(originalSeriesItemPartId);

    const disabledPartAssessmentId = "assessment-route-disabled-part";
    await store.saveAssessment(indexedRecord({
      assessmentId: disabledPartAssessmentId,
      actorStableId: "route-owner",
      createdAt: "2026-07-23T04:00:00.000Z",
      scopeType: "series",
      scopeId: seriesWithSku.id,
    }));
    const disabledPartState = structuredClone(workspace.state);
    disabledPartState.seriesDefinitions.find((series) => series.id === seriesWithSku.id)!.itemPartId = "part:hook";
    const disabledPartRevision = await saveWorkspaceState({
      state: disabledPartState,
      baseRevision: workspace.revision,
      author: "route-test",
      message: "disable retained assessment part",
    });
    assert.equal(disabledPartRevision.conflict, undefined);
    await assertOwnerSeesStaleHistory(disabledPartAssessmentId);
    assert.equal(
      (await getAssessment(
        request("GET", "other-user", disabledPartAssessmentId),
        context(disabledPartAssessmentId),
      )).status,
      404,
    );

    const inconsistentChainAssessmentId = "assessment-route-inconsistent-part-chain";
    await store.saveAssessment(indexedRecord({
      assessmentId: inconsistentChainAssessmentId,
      actorStableId: "route-owner",
      createdAt: "2026-07-23T05:00:00.000Z",
      scopeType: "series",
      scopeId: seriesWithSku.id,
    }));
    const inconsistentState = structuredClone(disabledPartState);
    inconsistentState.seriesDefinitions.find((series) => series.id === seriesWithSku.id)!.itemPartId = originalSeriesItemPartId;
    const inconsistentSku = inconsistentState.skuDrawers.find((sku) => sku.seriesId === seriesWithSku.id)!;
    inconsistentSku.projectionMatch.itemPartId = originalSeriesItemPartId === "part:rod"
      ? "part:reel"
      : "part:rod";
    const inconsistentRevision = await saveWorkspaceState({
      state: inconsistentState,
      baseRevision: disabledPartRevision.revision,
      author: "route-test",
      message: "break retained assessment part chain",
    });
    assert.equal(inconsistentRevision.conflict, undefined);
    await assertOwnerSeesStaleHistory(inconsistentChainAssessmentId);
    assert.equal(
      (await getAssessment(
        request("GET", "other-user", inconsistentChainAssessmentId),
        context(inconsistentChainAssessmentId),
      )).status,
      404,
    );
  } finally {
