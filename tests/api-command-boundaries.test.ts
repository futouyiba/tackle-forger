...
test("受治理边界覆盖命令、发布历史、审计与永久身份，且嵌套改动也不可绕过", () => {
  ...
  const cases: Array<{ field: string; action: string }> = [
    ...
    { field: "v23TechnologyDefinitions", action: "POST /api/v23/actions（create_technology / update_technology）" },
    { field: "v23TechnologyHeads", action: "POST /api/v23/actions（create_technology / update_technology）" },
    ...
    { field: "reductionStackingPolicyVersions", action: "POST /api/feishu-workbook（pull_feishu_workbook / publish_ruleset）" },
    ...
    { field: "canonicalRuleSourceDrafts", action: "POST /api/feishu-workbook（pull；create_ruleset_draft / publish_ruleset）" },
    { field: "weightTemplatePolicyDrafts", action: "POST /api/feishu-workbook（pull；create_ruleset_draft / publish_ruleset）" },
    ...
    { field: "aiRuleSourceChangeDrafts", action: "POST /api/ai/assessments/:assessmentId/drafts；POST /api/ai/rule-source-change-drafts/confirm" },
    { field: "aiArtifactProvenanceSyncRecords", action: "POST /api/ai/assessments/:assessmentId/drafts（创建草稿及永久来源同步/重试）" },
