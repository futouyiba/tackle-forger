...
test("01/02/03 同 revision 导入稳定 ID、显示值、规则和来源单元格", () => {
  ...
  assert.deepEqual(draft.functionProfiles.map((entry) => entry.id), ["func_0001", "func_0002"]);
  assert.deepEqual(
    draft.functionProfiles.map((entry) => entry.intensityRules.map((rule) => rule.sourceRowId)),
    [["func_0001"], ["func_0002"]],
  );
...
test("缺失源机器 ID 或未绑定钓法时 fail closed，显示名变化不会改变 Function identity", () => {
  ...
  assert.deepEqual(renamedDraft.functionProfiles.map((profile) => profile.id), ["func_0001", "func_0002"]);
  assert.deepEqual(renamedDraft.functionProfiles.map((profile) => profile.name), ["远投（改名）", "远投"]);
  assert.deepEqual(
    renamedDraft.functionProfiles.map((profile) => profile.intensityRules.map((rule) => rule.sourceRowId)),
    [["func_0001"], ["func_0002"]],
  );
});
