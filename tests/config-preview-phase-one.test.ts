...
test("同一 Model 的多个冻结修订不能进入同一配置预览", () => {
  const first = replayableSnapshot();
  const second = structuredClone(first);
  second.id = `${first.id}:revision-2`;
  const secondContent = structuredClone(second);
  Reflect.deleteProperty(secondContent, "contentHash");
  second.contentHash = deterministicHash(secondContent);

  assert.throws(
    () => createConfigPreviewPackage({
      packageId: "preview:duplicate-model",
      workspaceId: "workspace:test",
      snapshots: [first, second],
    }),
    (error) => error instanceof Error
      && "code" in error
      && error.code === "SNAPSHOT_MODEL_DUPLICATE",
  );
});
