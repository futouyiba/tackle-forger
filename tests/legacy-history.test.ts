...
test("历史目录引用删除后仍保留原始 ID 并明确标记未解析", () => {
  const currentCatalog = [{ id: "template:current", name: "当前模板" }];
  assert.deepEqual(
    resolveLegacyCatalogReference("template:current", currentCatalog, (entry) => entry.name),
    { id: "template:current", label: "当前模板", resolved: true },
  );
  assert.deepEqual(
    resolveLegacyCatalogReference("template:deleted", currentCatalog, (entry) => entry.name),
    { id: "template:deleted", label: null, resolved: false },
  );
});
...
test("根工作台不再重算历史 Candidate，规则图也没有下发写入口", async () => {
  ...
  assert.match(workbench, /ids=\{selected\.templateIds\}/);
  assert.match(workbench, /ids=\{candidate\.affixIds\}/);
  assert.match(workbench, /historicalDetails = state\.detailOverrides\.filter/);
  assert.match(workbench, /values=\{detail\.values\}/);
  assert.match(workbench, /历史精调和 OfficialSku 保留原始 key/);
  assert.match(workbench, /Excel 已导入；当前配置已载入，只读历史数据保持原样/);
  assert.match(workbench, /历史 Candidate 结果（兼容旧候选池）/);
  assert.doesNotMatch(workbench, /历史 Model 候选结果/);
});
