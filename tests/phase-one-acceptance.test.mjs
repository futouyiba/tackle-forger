...
const winSkip = { skip: isWindows ? "Windows 不支持 Unix 文件权限/symlink/npm spawn 语义" : undefined };
import {
...
test("错误租户、过期会话、旧 schema 17 与残缺工作簿全部保持 BLOCKED", async () => {
  const fetchImpl = mockFetch({
    "/api/auth/session": json({
      authenticated: true,
      user: {
...
test("稳定身份待确认或品质/定价草稿阻断时权威工作簿保持 BLOCKED", async () => {
  ...
  const fetchImpl = mockFetch({
    ...
    "/api/state": json({ revision: 17, state: { schemaVersion: 18 } }),
...
test("真实会话暴露正式提交或 AI Capability 时验收保持 BLOCKED", async () => {
  const fetchImpl = mockFetch({
    ...
    "/api/state": json({ revision: 1, state: { schemaVersion: 18 } }),
    "/api/revisions": json({ revisions: [] }),
    "/api/feishu-workbook": json({
      inspection: canonicalInspection(),
    }),
...
test("preflight 对安全 env、源契约和 0600 权限给出可重复证据", winSkip, async () => {
  ...
  await writeFile(
    path.join(root, "lib/config-preview-package.ts"),
    "interface ConfigPreviewPackage { packageKind: 'CONFIG_PREVIEW'; ref: 'NON_FORMAL:'; }\n",
  );
...
test("三种模式共用的环境 loader 拒绝仓库内、相对、symlink 与宽权限文件", winSkip, async () => {
...
test("npm 验收命令将缺失 env-file 交给脚本并返回去敏 BLOCKED 证据", winSkip, () => {
...
test("依赖门禁绑定受版本控制的 Issue/PR 映射、唯一 commit 与审核状态", async () => {
  ...
  const manifest = (dependencyCommits) => ({
    schemaVersion: "phase-one-dependency-evidence/v3",
    dependencies: [
  ...
  const githubPulls = new Map([
    [67, { merge: commits[0], head: commits[0], base: "f".repeat(40) }],
    [71, { merge: commits[1], head: commits[1], base: "f".repeat(40) }],
    [76, { merge: commits[2], head: commits[2], base: "f".repeat(40) }],
  ]);
  const fetchImpl = async (input) => {
    const url = new URL(input);
    if (url.pathname === "/graphql") {
      return json({
        data: { repository: { pullRequest: { reviewThreads: { nodes: [{ id: "thread", isResolved: true }], pageInfo: { hasNextPage: false, endCursor: null } } } } },
      });
    }
    if (url.pathname.endsWith("/actions/runs")) {
      const head = url.searchParams.get("head_sha");
      const [pr, expected] = [...githubPulls.entries()].find(([, value]) => value.head === head) ?? [];
      return json({ workflow_runs: [{
        id: pr * 100,
        event: "pull_request",
        head_sha: head,
        display_title: `gate-context event=pull_request pr=${pr} head=${head} base=${expected?.base}`,
        path: ".github/workflows/ci.yml",
        workflow_id: 318282943,
        run_attempt: 1,
        status: "completed",
        conclusion: "success",
      }] });
    }
    if (url.pathname.endsWith("/actions/workflows/.github/workflows/ci.yml")) {
      return json({ id: 318282943, path: ".github/workflows/ci.yml", state: "active" });
    }
    if (url.pathname.includes("/contents/.github/workflows/ci.yml")) {
      return json({
        type: "file",
        encoding: "base64",
    ...
    if (url.pathname.endsWith("/reviews")) {
      const pr = Number(url.pathname.split("/").at(-2));
      ...
      return json([{
        ...
        body: "Agent-Review: PASS",
        user: { login: `reviewer-${pr}`, type: "Bot" },
      }]);
    }
  ...
  // Historic runs named only "CI" are not trusted.  A later, merged PR may
  // attest them only when its own immutable gate-context evidence is verified.
  await writeFile(path.join(root, "successor.txt"), "successor\n");
  execFileSync("git", ["add", "successor.txt"], { cwd: root });
  execFileSync("git", ["commit", "-m", "fixture successor attestation (#90)"], { cwd: root });
  const successorCommit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
  githubPulls.set(90, { merge: successorCommit, head: successorCommit, base: commits[2] });
  const legacyOriginalRun = async (input) => {
    const url = new URL(input);
    if (!url.pathname.endsWith("/actions/runs")) return fetchImpl(input);
    const head = url.searchParams.get("head_sha");
    if (head !== commits[0]) return fetchImpl(input);
    return json({ workflow_runs: [
      {
        id: 6700, event: "pull_request", head_sha: head, name: "CI",
        display_title: "fix: preserve schema v17 compatibility on main",
        path: ".github/workflows/ci.yml", workflow_id: 318282943,
        status: "completed", conclusion: "success",
      },
      {
        id: 6701, event: "pull_request", head_sha: head, name: "CI",
        display_title: "fix: enforce phase-one NON_FORMAL config previews (#72)",
        path: ".github/workflows/ci.yml", workflow_id: 318282943,
        status: "completed", conclusion: "success",
      },
    ] });
  };
  assert.equal((await inspectDependencyManifest(root, {
    fetchImpl: legacyOriginalRun, githubToken: "test-token",
  })).status, "BLOCKED");
  const attested = manifest(commits);
  attested.dependencies[0].successorAttestation = {
    pr: 90, headCommit: successorCommit, baseCommit: commits[2], mergeCommit: successorCommit,
  };
  await writeFile(path.join(root, "deploy/phase-one-dependencies.json"), `${JSON.stringify(attested, null, 2)}\n`);
  assert.equal((await inspectDependencyManifest(root, {
    fetchImpl: legacyOriginalRun, githubToken: "test-token",
  })).status, "PASS");
  const mixedLegacyAndStructured = async (input) => {
    const url = new URL(input);
    if (!url.pathname.endsWith("/actions/runs") || url.searchParams.get("head_sha") !== commits[0]) {
      return legacyOriginalRun(input);
    }
    const payload = await (await legacyOriginalRun(input)).json();
    payload.workflow_runs.push({
      id: 6701, event: "pull_request", head_sha: commits[0], name: "CI",
      display_title: `gate-context event=pull_request pr=67 head=${commits[0]} base=${"e".repeat(40)}`,
      path: ".github/workflows/ci.yml", workflow_id: 318282943,
      status: "completed", conclusion: "success",
    });
    return json(payload);
  ...
  assert.equal((await inspectDependencyManifest(root, {
    fetchImpl: mixedLegacyAndStructured, githubToken: "test-token",
  })).status, "BLOCKED");
  await writeFile(path.join(root, "deploy/phase-one-dependencies.json"), `${JSON.stringify(manifest(commits), null, 2)}\n`);
  ...
  const staleBase = await inspectDependencyManifest(root, {
    githubToken: "test-token",
    fetchImpl: async (input) => {
      const response = await fetchImpl(input);
      const url = new URL(input);
      if (!url.pathname.endsWith("/actions/runs")) return response;
      const payload = await response.json();
      return json({
        workflow_runs: payload.workflow_runs.map((run) => ({
          ...run,
          display_title: run.display_title.replace(/base=[a-f0-9]{40}$/u, `base=${"e".repeat(40)}`),
        })),
      });
    },
  });
  assert.deepEqual(staleBase.evidence.githubWorkflowTrustVerificationFailedIds, [
    "canonical_rule_source", "schema_v17", "non_formal_preview",
  ]);

  const nonCanonicalWorkflow = await inspectDependencyManifest(root, {
    githubToken: "test-token",
    fetchImpl: async (input) => {
      const response = await fetchImpl(input);
      const url = new URL(input);
  ...
  assert.deepEqual(
    nonCanonicalWorkflow.evidence.githubWorkflowTrustVerificationFailedIds,
    ["canonical_rule_source", "schema_v17", "non_formal_preview"],
  );

  const attemptJobPaths = [];
  assert.equal((await inspectDependencyManifest(root, {
    githubToken: "test-token",
    fetchImpl: async (input) => {
      const url = new URL(input);
      if (url.pathname.includes("/actions/runs/") && url.pathname.endsWith("/jobs")) {
        attemptJobPaths.push(url.pathname);
      }
      return fetchImpl(input);
    },
  })).status, "PASS");
  assert.equal(attemptJobPaths.length, 3);
  assert.ok(attemptJobPaths.every((value) => value.includes("/attempts/1/jobs")));

  const missingReview = await inspectDependencyManifest(root, {
    githubToken: "test-token",
    fetchImpl: async (input) => new URL(input).pathname.endsWith("/reviews")
      ? json([])
      : fetchImpl(input),
  });
  assert.deepEqual(
    missingReview.evidence.githubCurrentHeadReviewVerificationFailedIds,
    ["canonical_rule_source", "schema_v17", "non_formal_preview"],
  );

  const changesRequested = await inspectDependencyManifest(root, {
    githubToken: "test-token",
    fetchImpl: async (input) => {
      const url = new URL(input);
      if (!url.pathname.endsWith("/reviews")) return fetchImpl(input);
      const pr = Number(url.pathname.split("/").at(-2));
      const head = githubPulls.get(pr)?.head;
      return json([
        {
          id: pr,
          state: "COMMENTED",
          ...
          submitted_at: "2026-07-23T00:00:00Z",
          body: "Agent-Review: PASS",
          user: { login: "same-reviewer", type: "Bot" },
        },
        {
          id: pr + 1000,
  ...
  assert.deepEqual(
    changesRequested.evidence.githubCurrentHeadReviewVerificationFailedIds,
    ["canonical_rule_source", "schema_v17", "non_formal_preview"],
  );
  ...
  const pushOnly = await inspectDependencyManifest(root, {
    ...
    fetchImpl: async (input) => new URL(input).pathname.endsWith("/actions/runs")
      ? json({ workflow_runs: [{
        ...
        event: "push",
        head_sha: commits[0],
        display_title: `gate-context event=pull_request pr=67 head=${commits[0]} base=${"f".repeat(40)}`,
        path: ".github/workflows/ci.yml",
        workflow_id: 318282943,
        run_attempt: 1,
        status: "completed",
...
test("持久数据根检查要求正确类型、仅服务账号权限与安全父目录", winSkip, async () => {
...
test("会话 Cookie 文件必须是仓库外绝对路径、0600 普通文件且不能是 symlink", winSkip, async () => {
...
test("evidence output 可以创建为独立 0600 文件且不覆盖旧证据", winSkip, async () => {
