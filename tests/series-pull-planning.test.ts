...
test("范围内候选只形成建议，确认后才逐拉力物化 SKU 并要求独立 ProjectionMatch", () => {
  ...
  assert.equal(state.skuDrawers.some((sku) => sku.targetWeightKg === 3.8), false);
  const match = {
  ...
  assert.equal(result.skus.find((sku) => sku.id === "sku:qinglu-obstacle-3.8")?.projectionMatch.targetWeightKg, 3.8);
  assert.equal(result.skus.filter((sku) => sku.seriesId === series.id && sku.targetWeightKg === 3.8).length, 1);
});
...
test("过期规划建议不能覆盖新 revision，已存在拉力不会重复创建", () => {
  ...
  assert.throws(() => materializeConfirmedPullSpecifications({
    ...
    projectionMatchByPull: { "3.8": { ...state.skuDrawers[0].projectionMatch, targetWeightKg: 3.8 } },
    createdAt: "2026-07-22T01:05:00.000Z",
  }), /revision 已过期/);
  ...
  assert.equal(unchanged.skus.filter((sku) => sku.seriesId === series.id && sku.targetWeightKg === 1.5).length, 1);
});
