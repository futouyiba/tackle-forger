...
test("superseded 历史 SKU 不占用当前拉力，但其稳定 ID 永久禁止复用", () => {
  const state = createSeedState();
  const series = structuredClone(state.seriesDefinitions[0]!);
  ...
  const existingSkus = state.skuDrawers.map((sku) =>
    sku.id === historicalSku.id ? historicalSku : structuredClone(sku));
  const proposal = createSeriesPullPlanningProposal({
    series,
    existingSkus,
    candidatePullsKgf: [1.5],
    source: "explicit_user_input",
    createdAt: "2026-07-23T03:20:00.000Z",
  });
  const replacementSkuId = "sku:qinglu-obstacle-1.5-rematerialized";
  const projectionMatch = {
    ...historicalSku.projectionMatch,
    targetPullKg: 1.5,
  };
  ...
  const result = materializeConfirmedPullSpecifications({
    ...
    proposal,
    confirmedPullsKgf: [1.5],
    skuIdByPull: { "1.5": replacementSkuId },
    projectionMatchByPull: { "1.5": projectionMatch },
    createdAt: "2026-07-23T03:21:00.000Z",
  });

  assert.deepEqual(result.createdSkuIds, [replacementSkuId]);
  assert.deepEqual(
    result.series.targetPullSpecifications.find((entry) => entry.targetPullKgf === 1.5),
    { targetPullKgf: 1.5, skuId: replacementSkuId },
  );
  assert.deepEqual(
    result.skus.find((sku) => sku.id === historicalSku.id),
    historicalSku,
  );
  assert.throws(() => materializeConfirmedPullSpecifications({
    series,
    ...
    proposal,
    confirmedPullsKgf: [1.5],
    skuIdByPull: { "1.5": historicalSku.id },
    projectionMatchByPull: { "1.5": projectionMatch },
    createdAt: "2026-07-23T03:21:00.000Z",
  }), /已存在，禁止重复物化/);
});
