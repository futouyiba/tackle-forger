...
test("ERROR 默认不可 waive，只有完整且精确命中的已发布策略可以原子批准", () => {
  ...
  assert.throws(
    () => approveValidationWaiverDecision({
      ...command,
      idempotencyKey: "waiver:duplicate-target",
      requestedWaivers: [command.requestedWaivers[0], command.requestedWaivers[0]],
    }),
    /只能在一个 Waiver 决定中请求一次/,
  );
  ...
  const tamperedWaiver = structuredClone(approved.waivers[0]);
  tamperedWaiver.issueFingerprint = "tampered-fingerprint";
  const tamperedContent: Record<string, unknown> = { ...tamperedWaiver };
  delete tamperedContent.state;
  delete tamperedContent.stateHash;
  delete tamperedContent.stateHashVersion;
  delete tamperedContent.recordHash;
  tamperedWaiver.recordHash = deterministicHash(tamperedContent);
  tamperedWaiver.stateHash = deterministicHash({
    stateHashVersion: tamperedWaiver.stateHashVersion,
    recordHash: tamperedWaiver.recordHash,
    state: tamperedWaiver.state,
  });
  assert.throws(
    () => approveValidationWaiverDecision({
      ...command,
      issues: approved.issues,
      existingDecisions: [approved.decision],
      existingWaivers: [tamperedWaiver, approved.waivers[1]],
    }),
    /原子 Waiver 集合未完整冻结/,
  );
...
test("输入变化后旧 Issue、确认与 Waiver 一起 STALE，旧证据不被改写删除", () => {
  ...
  assert.deepEqual(invalidatedRetry.issues, invalidated.issues);
...
test("新正式 Snapshot 只接受并冻结指纹绑定的确认记录，旧 code 理由字典不能替代证据", () => {
  ...
  const publishError = issue({
    code: "PUBLISH_WAIVER_REQUIRED",
    source: "publish",
    subjectRef: {
      workspaceId: "workspace:1",
      entityType: "model",
    ...
    ruleRefs: [projection.ruleSetVersion],
    inputHash: "publish-waiver-input",
  });
  const publishPolicy = createWaiverPolicyVersion({
    policyId: "validation-waiver-policy",
    version: "validation-waiver/publish-snapshot-v1",
    status: "PUBLISHED",
    publishedAt: "2026-07-23T02:00:00.000Z",
    rules: [{
      source: "publish",
  ...
  const publishWaiver = approveValidationWaiverDecision({
    issues: [publishError],
    requestedWaivers: [{
      issueFingerprint: publishError.fingerprint,
      expectedIssueRevision: publishError.issueRevision,
      expectedInputHash: publishError.inputHash,
      gate: "PUBLISH",
    }],
    ...
    idempotencyKey: "waiver:snapshot-decision",
    capabilities: [APPROVE_VALIDATION_WAIVER_CAPABILITY],
  });
  assert.throws(
    () => publishConfigurationSnapshot({
  ...
  const legacyExportOnly = {
    ...
    gate: "EXPORT" as const,
    state: "OPEN" as const,
    code: "LEGACY_EXPORT_ONLY",
    message: "只约束导出。",
  };
  const publishWithLegacyExportOnly = publishConfigurationSnapshot({
    ...common,
    publicationMode: "historical_import",
    snapshotId: "snapshot:legacy-export-only",
    validationReport: [legacyExportOnly],
    warningConfirmations: {},
  });
...
test("ExportManifest 只冻结精确环境×渠道命中的统一 Issue 与证据引用", () => {
  ...
  const legacyTargetedSnapshot = structuredClone(snapshot);
  legacyTargetedSnapshot.validationReport = [{
    level: "error",
    severity: "ERROR",
    gate: "EXPORT",
    state: "OPEN",
    code: "LEGACY_TARGETED_EXPORT",
    message: "冻结为其他导出目标。",
    environmentId: "dev",
    channelKey: "1001",
  }];
  ...
  legacyUntargetedSnapshot.validationReport = [{
    ...
    gate: "EXPORT",
    state: "OPEN",
    code: "LEGACY_UNTARGETED_EXPORT",
    message: "缺少冻结导出目标。",
  }];
  const legacyUntargetedContent = structuredClone(legacyUntargetedSnapshot);
  Reflect.deleteProperty(legacyUntargetedContent, "contentHash");
  legacyUntargetedSnapshot.contentHash = deterministicHash(legacyUntargetedContent);
  assert.throws(
  ...
  malformedCanonicalSnapshot.validationReport = [{
    ...exportWarning,
    environmentId: undefined,
    channelKey: undefined,
  }];
  const malformedCanonicalContent = structuredClone(malformedCanonicalSnapshot);
  Reflect.deleteProperty(malformedCanonicalContent, "contentHash");
  malformedCanonicalSnapshot.contentHash = deterministicHash(malformedCanonicalContent);
  assert.throws(
    () => createExportManifest({
      ...common,
      snapshot: malformedCanonicalSnapshot,
    }),
    /Canonical ValidationIssue .*目标不完整/,
  );
  assert.throws(
    () => createExportManifest({
      ...common,
  ...
  const exportError = issue({
    code: "EXPORT_WAIVER_REQUIRED",
    source: "config_relationship",
    gate: "EXPORT",
    environmentId: "production",
    channelKey: "steam",
  });
  const exportPolicy = createWaiverPolicyVersion({
    policyId: "validation-waiver-policy",
    version: "validation-waiver/export-manifest-v1",
    status: "PUBLISHED",
    publishedAt: "2026-07-23T02:00:00.000Z",
    rules: [{
      source: "config_relationship",
  ...
  const exportWaiver = approveValidationWaiverDecision({
    issues: [exportError],
    requestedWaivers: [{
      issueFingerprint: exportError.fingerprint,
      expectedIssueRevision: exportError.issueRevision,
      expectedInputHash: exportError.inputHash,
      gate: "EXPORT",
      environmentId: "production",
      channelKey: "steam",
    }],
    policy: exportPolicy,
    scopeRef: exportError.subjectRef,
    reason: "批准目标导出例外。",
    approvedBy: "publisher:1",
    approvedAt: "2026-07-23T03:30:00.000Z",
    idempotencyKey: "waiver:export-manifest",
    capabilities: [APPROVE_VALIDATION_WAIVER_CAPABILITY],
  });
  ...
  assert.throws(
    () => createExportManifest({
      ...common,
      snapshot: snapshotWithExportError,
      validationGovernance: {
        issues: exportWaiver.issues,
        waivers: exportWaiver.waivers,
        activeWaiverPolicies: [exportPolicy],
      },
    }),
    /ValidationWaiverDecision/,
  );
  const waiverManifest = createExportManifest({
    ...common,
    snapshot: snapshotWithExportError,
    validationGovernance: {
      issues: exportWaiver.issues,
  ...
  assert.deepEqual(waiverManifest.validationWaiverRefs, [exportWaiver.waivers[0].waiverId]);
  assert.deepEqual(
    waiverManifest.validationWaiverDecisionRefs,
    [exportWaiver.decision.waiverDecisionId],
  );
});
